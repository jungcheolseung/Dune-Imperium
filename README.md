...
v79이며 기본 룰셋은 4,152개, CHOAM 룰셋은 4,429개다. 전체 테스트 842개, Ruff,
mypy가 통과한다.
...
leader draft 설정부터 실제 보드 이미지를 사용하는 한 화면 게임 테이블·카드
이미지·행동 선택·최종 순위까지 한 게임을
완주할 수 있고, 같은 API를 JSON으로도 쓸 수 있다. 진행 중이거나 끝난
게임은 `GameReplay` 직렬화 기반 로컬 저장 파일(`--saves-dir`, 기본
`~/.dune-imperium/saves`)로 저장·불러오기가 되며, 불러온 게임은 저장하지
않은 세션과 동일하게 이어진다. 끝난 게임은 사람 좌석의 `PlayerView` 시점을
step 단위로 되돌려 보는 replay 검토 화면으로 복기할 수 있다.
...
- 사람용 UI의 추가 조작 개선(형태는 로컬 웹 UI로 확정, 2026-08-30;
  2026-08-31에 효과 정보와 카드 이미지, 보드 이미지 기반 한 화면 테이블을
  추가했다. 보드 원본은 재배포하지 않고 로컬 `images/map.jpg`에서 선택적으로
  제공한다)
...
보드 및 카드 원본 이미지는 저작권과 재배포 범위를 고려해 Git 저장소에 넣지
않는다. 저장소의 `.gitignore`는 다음 두 로컬 이미지 디렉터리를 제외한다.

- `images/`: 사용자 제공 게임판 이미지. 한 화면 게임 테이블은
  `images/map.jpg`를 중앙 보드로 사용하며 서버가 `/game-images/map.jpg`로
  로컬에서만 제공한다. 다른 위치를 쓰려면 `DUNE_IMPERIUM_GAME_IMAGE_DIR`에
  해당 디렉터리를 지정한다. 이미지가 없으면 규칙 엔진과 텍스트 UI는 동작하지만
  중앙 보드 배경은 표시되지 않는다.
- `downloads/dunecardshub/cards/`: 카드·Leader·Contract·Conflict·보드 공간
  이미지 캐시. 새 환경에서는 아래 명령으로 필요한 WebP 166장을 준비한다.

```bash
uv run scripts/fetch_card_images.py
```

두 디렉터리의 파일은 로컬 플레이에만 사용하고 commit하거나 재배포하지 않는다.
새 이미지가 필요하면 사용 권한을 확인한 뒤 알맞은 로컬 디렉터리에 추가한다.
