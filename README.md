...
코어 엔진은 Endgame Intrigue window(OQ-001 convention)까지 갖춰 random 4인 게임을 FINISHED까지 완주하고 replay할 수 있다. Intrigue 44장은 39개 identity 전부가 effect DSL로 play되어 완결됐고, 인쇄된 Leader 9종(기본 8 + CHOAM 전용 Shaddam)의 능력과 Signet Ring도 모두 play된다. `run_random_game`/`run_policy_game` 러너와 `dune_imperium_uprising_v1` PettingZoo adapter는 전체 게임을 하나의 episode로 실행하며, 관측은 버전이 붙은 4,327-int 전체 게임 인코딩(v20; Reveal 전에도 배치 유닛 기준 전투력을 담고 Bloodlines Commander·Skill·Tech tile과 Immortality의 Bene Tleilax board 세그먼트를 포함한다), 보상은 승자독식 zero-sum 종료 보상이다 ([학습 환경 설계](docs/rl-environment.md)). AI 상대로는 seeded random 외에 M11의 규칙 기반 `HeuristicAgent`가 있으며(보드 공간 표는 2026-09-16의 축별 A/B와 소거로 재정비, [baseline-2026-09-10.md](docs/evaluation/baseline-2026-09-10.md) 13~18절; 같은 날 밤 동점 census로 찾은 Influence 진영·trash 카드·같은 비용 구매의 가족 안 tie-break를 채택해 고정 변형 상대 전 룰셋 +5 ~ +9%p, [baseline-2026-09-16.md](docs/evaluation/baseline-2026-09-16.md) 14절), 검증 sweep은 `--policy {random,heuristic}`으로 두 baseline을 모두 완주 검사할 수 있다. M9의 `dune-imperium-tournament`는 이름 붙인 baseline들을 좌석 회전·Leader 회전·seed 범위로 교차 대전시켜 승률·평균 순위·VP margin·좌석/Leader/선공별 성적·불법 행동·결정 시간을 Markdown/JSON으로 보고한다(현행 기준선: [docs/evaluation/baseline-2026-09-16.md](docs/evaluation/baseline-2026-09-16.md), heuristic이 random 3명 상대 98.4% 승률/800판; 이전 기준선은 [2026-09-10판](docs/evaluation/baseline-2026-09-10.md)(heuristic 보드 공간 표의 A/B 기록 13~18절 포함)과 [2026-09-06판](docs/evaluation/baseline-2026-09-06.md)). 첫 search baseline `RolloutAgent`는 자기 좌석이 아는 정보만으로 숨겨진 존을 다시 나눈 세계(`agents/determinize.py`)에서 후보 행동마다 heuristic rollout을 돌려 고르며, heuristic 3명 상대 54.0% 승률(평균 순위 1.74)이고 확장을 전부 켜면 60.0%다(2026-09-16 기준선 요약 표의 마지막 열, tie-break heuristic 상대, 15절의 값 함수 채택 뒤; 강해진 heuristic 앞에서 28%까지 내려왔던 rollout을 같은 예산에서 세계 4 · 후보 3 · 공통 난수로 재정비한 값이며, 두 배 예산의 `rollout_strong`은 base에서 60%다). M10 진입점인 `dune-imperium-selfplay`(`training/` 패키지)는 여러 게임을 lockstep으로 돌리며 좌석별 정책의 추론을 batch로 묶고, zero-sum 종료 보상과 관측·mask·행동 궤적을 NumPy 배열로 내놓는다(단일 프로세스 약 4,900 decisions/s). M10 첫 슬라이스 `dune-imperium-train`(`train` extra, PyTorch)은 masked policy/value MLP를 self-play 종료 보상으로 학습(REINFORCE + value baseline)하고, 관측·codec 버전을 기록한 체크포인트를 저장하며, 체크포인트는 `checkpoint:<경로>` 이름으로 대회 도구에 바로 들어간다. 이 Mac CPU에서 6분(self-play 960판) 학습한 첫 체크포인트가 random 3명 상대 100%, heuristic 3명 상대 56% 승률로 당시 rollout(48%; 현행 기준선에서는 54.0%)을 넘었고, 8 worker 병렬 수집으로 14분(6,400판) 학습한 두 번째 체크포인트는 heuristic 상대 89%, 첫 체크포인트 상대 61%, rollout 상대 67%이며, 거기서 100 iteration을 이은 세 번째 체크포인트는 두 번째 상대 35%(동률 25%), heuristic 상대 91%다. 이 승률 세 묶음은 모두 **관측 v4 체크포인트 기준**이고 현행 로더(관측 v20·codec v104)가 그 체크포인트를 거부하므로 M10 재개 시 새로 측정한다; 권위 있는 현행 기준선은 위에 적은 [docs/evaluation/baseline-2026-09-16.md](docs/evaluation/baseline-2026-09-16.md)다. Leader 선택은 OQ-007의 6종 공개 draft convention을 `RulesetConfig(leader_draft=True)` 옵션(sweep `--leader-draft`)으로 켤 수 있다(공식 규칙 아님; 기본은 고정 배정). 정식 덱 밖의 Uprising 프로모 Imperium 3장(Arrakis Revolt, The Beast's Spoils, Pivotal Gambit)은 `RulesetConfig(promo_cards=True)` 옵션(sweep `--promo-cards`, UI 체크박스)으로 덱에 넣으며, 카드면만이 출처라 공식 문서가 침묵하는 판정은 OQ-024~026에 project convention으로 기록했다. Bloodlines 프로모 Ruthless Leadership은 같은 옵션에 `bloodlines=True`를 함께 켤 때 덱에 들어간다(2026-09-08). Bloodlines 확장은 `RulesetConfig(bloodlines=True)`(sweep `--bloodlines`, UI 체크박스)로 켜며 2026-09-07 현재 Sardaukar Commander와 Skill(획득·지불 recruit·배치·strength·Reveal 보너스·정리·기존 troop 효과 포함), Conflict 카드 2장, wild끼리의 Endgame 매칭, Tech Module 전용을 뺀 Imperium 카드 25종(Command (6+)·Combat 아이콘·Spy with Deep Cover 포함)과 Intrigue 16장, Leader 8종(Chani·Fenring·Duncan Idaho·Esmar Tuek과 Tuek's Sietch·Mohiam·Liet Kynes·Piter De Vries와 Twisted Intrigue 12장·Steersman Y'rkoon과 Navigation 10장), 그리고 `tech_module=True`(sweep `--tech-module`)로 켜는 Tech Module(Ixian Embassy와 Tech tile 18장의 획득·할인·Flip·능력, Tech 전용 Imperium 2장·Intrigue 2장, Kota Odax of Ix)까지 구현돼 있어 Bloodlines 구성물 전부가 play된다([docs/rules/bloodlines.md](docs/rules/bloodlines.md); 공식 문서가 침묵하는 판정은 OQ-031~047; Endgame tiebreaker의 garrison troop 수에는 Commander를 포함한다). 2026-09-07 밤의 슬라이스 7로 웹 UI가 Commander·Skill·Ixian Embassy·보유 Tech tile을 표시하고, heuristic이 tile별 가치로 Tech를 사며, `--bloodlines --tech-module` 교차 소크 7,000판(random 4,000·heuristic 2,000·draft 1,000, `--soundness-interval 25`)이 실패 0으로 통과하고 관측 v9·codec v94 기준 학습 루프 smoke도 끝나 M12는 완료다(소크가 적발한 엔진 결함 3계열은 OQ-022·038 보강과 OQ-046으로 수정). M11 사람용 플레이(완료)는 FastAPI 기반 로컬 서버(`uv sync --extra rl --extra ui` 후 `uv run dune-imperium-server`, 기본 http://127.0.0.1:8000)로 한다: 브라우저에서 좌석 배정(사람, 또는 AI 종류: 휴리스틱·롤아웃 탐색·랜덤·학습 체크포인트 경로)·CHOAM·leader draft 설정부터 한 화면 게임 테이블(로컬 보드 스캔 위에 합법 공간 발광·Agent 토큰·Control·Spy를 겹쳐 그리고, 카드는 인쇄 이미지, 효과는 공식 룰북에서 추출한 아이콘으로 표시; 스캔·이미지·아이콘이 없으면 텍스트로 대체)·행동 선택·최종 순위까지 한 게임을 완주할 수 있고, 같은 API를 JSON으로도 쓸 수 있다. 진행 중이거나 끝난 게임은 `GameReplay` 직렬화 기반 로컬 저장 파일(`--saves-dir`, 기본 `~/.dune-imperium/saves`)로 저장·불러오기가 되며, 불러온 게임은 저장하지 않은 세션과 동일하게 이어진다. 끝난 게임은 모든 좌석 시점을 step 단위로 되돌려 보는 replay 검토 화면으로 복기할 수 있고, 종료 후에는 모든 비공개 존이 공개된다(OQ-010). 진행 중에는 실시간 행동 로그(이벤트를 좌석별 가시성으로 걸러 표시)와 행동 되돌리기(자기 연속 행동만, 무작위 결과·다른 좌석 행동·숨겨진 정보의 공개 이후로는 불가; 되돌린 행동은 로그에 남음)를 쓸 수 있다. 각 행동에는 되돌리기 가능 여부가 표시되고, 되돌릴 수 있는 행동으로 턴이 끝나면 플레이어가 턴 종료를 확정하기 전까지 다음 좌석으로 넘어가지 않는다. 보드 스캔 위에는 Influence 큐브·VP·전투력·High Council 마커와, 인쇄된 자리에 놓이는 조각들(칸 아래 깃발의 Control marker, Maker 육각형의 bonus spice 수량, garrison 옆 슬롯의 Maker Hooks token, 진영 strip의 원에 있다가 획득한 좌석 패널로 옮겨 가는 Alliance token), 모서리 원의 garrison과 사분면의 Conflict 배치 병력, 인쇄된 슬롯의 Conflict 카드·Conflict 덱·face-up Contract도 그려지고, 카드와 공간은 마우스를 올리면 상세가 뜬다. 공용 카드 영역은 보드 옆 세로 열에, 행동 로그는 행동 목록 옆에 좌석별 턴 카드(게임 진행 이벤트는 별도 카드)로 놓이며 카드 위에 마우스를 올리면 관련 공간이 보드에서 켜진다. 새로 놓인 Agent 토큰은 좌석 패널에서 보드 공간으로 날아와 자리잡고, 1700px 이하 화면에서는 오른쪽 패널이 1열로 쌓인다. 머리글의 **English / 한국어** 버튼이 화면 언어를 바꾸고(카드·리더·공간 이름과 인쇄 텍스트는 두 언어 모두 영어, 규칙 용어는 공식 한국어 룰북 용어집을 따른다), **도움말**(`?`)이 아이콘 범례·단축키·한 턴 진행을 보여 주며, 차례 변경과 오류는 스크린 리더에도 읽힌다. 끝난 판의 배너는 승자와 동점 판정 근거를 말한다.

M14 원격 멀티플레이(원격의 친구들과 각자 PC에서 한 판; [설계](docs/multiplayer-design.md), 2026-09-17 확정)는 진행 중이다. 슬라이스 1(2026-09-17)로 서버에 접근 계층이 생겼다: `uv run dune-imperium-server --remote`는 방 링크 하나로 들어온 사람이 빈 사람 좌석을 claim하게 하고(좌석 자격은 그 브라우저의 HttpOnly 쿠키에만 있다), 게임 생성·저장·불러오기·삭제는 시작할 때 콘솔에 찍히는 관리자 링크로만 허용하며, 결정론적 엔진에서 seed는 곧 모든 덱 순서이므로 게임이 끝날 때까지 seed를 어떤 응답에도 싣지 않는다. loopback이 아닌 `--host`는 `--remote` 없이는 시작을 거부한다. 옵션 없이 띄운 로컬 서버는 예전과 똑같이 동작한다. 슬라이스 2(같은 날)로 화면 갱신이 요청 하나가 됐다: `GET /games/{id}/snapshot`이 summary·좌석 view·합법 행동·로그의 새 항목만을 한 상태에서 읽어 주고 응답은 gzip으로 나가서, 전 확장 한 판 기준 갱신 한 번이 4요청·120.7 KB에서 1요청·14.6 KB(gzip 3.3 KB)로, 한 판 누적이 64.8 MB에서 8.3 MB(gzip 1.81 MB)로 줄었다(혼자 하는 로컬 판도 같은 경로를 쓴다; `uv run python scripts/measure_server_payloads.py`로 재현). 슬라이스 3(같은 날)으로 실시간 갱신이 붙었다: 게임이 바뀔 때마다 서버가 `GET /games/{id}/events`(Server-Sent Events)로 공개 필드만 담은 "초인종"을 울리고, 브라우저는 자기 화면이 낡았을 때만 snapshot을 다시 받는다(다른 브라우저의 행동이 약 0.2초 안에 반영되고, 자기 행동에는 추가 요청이 없다). 스트림이 통하지 않는 프록시 뒤에서는 2초 간격 폴링으로 스스로 전환한다. 열린 스트림이 곧 접속 표시(`players[].online`)다. 슬라이스 4(같은 날)로 브라우저 UI가 원격 흐름을 갖췄다: 호스트는 콘솔의 관리자 링크로 들어와 방을 만들고 호스트 패널의 방 링크(`--public-url`로 친구들이 접속하는 주소를 지정) 하나를 보내며, 친구는 그 링크에서 이름을 적고 빈 좌석에 앉는다. 화면은 자기 좌석 시점으로 고정되고, 남의 차례에는 "좌석 N (이름) 결정 대기 중"과 접속 점이, 자기 차례에는 탭 제목과 짧은 신호음이 알린다. remote 서버에서는 앞 좌석이 턴 종료를 확정하기 전에는 다음 좌석이 행동할 수 없다. 새로고침·탭 닫기 뒤에는 같은 링크(또는 첫 화면의 "이어 하기")로 같은 좌석에 돌아오고, 호스트는 좌석을 비워 다른 사람이 앉게 할 수 있다. 이 흐름과 기존 로컬 흐름의 브라우저 E2E는 [`scripts/e2e/`](scripts/e2e/README.md)에 있다(Playwright는 프로젝트 의존성이 아니다). 슬라이스 5(같은 날)로 remote 서버는 턴이 넘어갈 때마다 게임당 하나의 자동 저장을 저장 폴더에 갈아 끼운다(`--no-autosave`로 끔). 서버가 죽으면 다시 띄우고 관리자 링크로 들어가 저장 목록의 "자동 저장"을 불러온 뒤 새 방 링크를 보내면 되고, 잃는 것은 진행 중이던 한 턴이다. 기다리던 친구의 화면에는 "서버 연결 끊김"이, 서버가 돌아오면 "호스트의 새 방 링크로 들어오세요"가 뜬다. 슬라이스 6(같은 날)으로 호스트·친구용 절차를 [원격 한 판 운영 안내](docs/remote-play-guide.md)에 적었고, 이 Mac mini의 Tailscale 주소에 실제로 bind해 리허설했다(평문 HTTP origin에서의 쿠키·링크 복사, 회선 에뮬레이션: 가정 회선에서 첫 접속 1.8초·한 수의 반영 29 ms). 남은 것은 다른 집의 친구와 실제로 하는 한 판과 그 피드백이다.
...
- 사람용 UI의 추가 조작 개선(형태는 로컬 웹 UI로 확정, 2026-08-30; 구현 순서도 M11을 M9·M10보다 앞으로 조정했다. 2026-08-31에 효과 정보 전면 표시를, 2026-09-03에 보드 스캔 기반 한 화면 테이블과 룰북 아이콘 표시를 추가했다. 2026-09-06 저녁에 호버 팝오버, 보드 슬롯의 Conflict·Contract·덱, 병력 칩, 카드 열 세로 배치, 행동 로그 턴 카드를 추가했다. 2026-09-20에 전 좌석 AI 게임의 관전을 추가했다 — 사람 좌석 없이 만든 게임은 리플레이 검토가 처음부터 자동 재생으로 열린다(기본 한 턴씩 · 1초, 한 수씩·간격 선택, 행동 로그가 커서를 따라간다). Agent 토큰 이동 애니메이션 등은 열려 있다. 2026-09-21에 종료 화면·좌석 스탯·접근성·도움말·한국어/영어 전환을 더했다 — [UI 개선 계획](docs/ui-improvement-plan.md) 5단계)
...
카드·보드·아이콘·토큰 원본은 저작권 때문에 이 저장소에 넣지 않는다. 플레이 서버는
아래 네 위치를 있을 때만 서빙하고, 없으면 텍스트(및 텍스트 보드 목록, 그려 넣은 좌석 토큰)로
...
- `assets/tokens/` (또는 `DUNE_IMPERIUM_TOKEN_DIR`): 좌석 색상별 Combat marker
  그림 8장(`strength_<blue|red|green|yellow>.png`과 `+20` 면
  `strength_<색>_plus20.png`; 이름 규칙은 `dune_imperium.display.token_images`).
  보드 하단 Combat track의 전투력 토큰이 이 그림으로 바뀌고, 전투력이 20을
  넘으면 `+20` 면으로 뒤집힌다[Main p. 12]. 한 색의 두 면이 다 있어야 그 좌석에
  적용된다. 같은 폴더의 `shield_wall.png`는 Shield Wall 토큰으로, 제거되기 전까지
  Spice Refinery 아래의 표시된 위치에 놓인다[Main p. 4]. 출처는 에셋 저장소 README 참고.
