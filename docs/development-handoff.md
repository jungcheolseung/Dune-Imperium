...
밤에 recruit 집계 전수 수정과 OQ-064 판정(Coercive Negotiation은 contract bank 3장 미만이면 아예 사용 불가)도 병합했다.
OQ-065(강제 Spy 이동 순서와 갈 곳이 없는 Spy)도 밤에 사용자 판정으로 확정했고, 자기 자신을 trash하는 경로 9곳에 trash-recruit guard를 예방적으로 넣었다(아래 요약). 그 뒤(2026-09-27
새벽) OQ-070으로 이번 turn에 recruit한 Commander의 배치 몫은 Commander만 쓰게 고쳤다(아래 요약). 이로써 이 날 사용자에게 물은 판정은
모두 정해졌다(OPEN 없음). 같은 새벽 Agent box의 Spy도 supply가 비면 box를 해결하는 그 시점에 선행 recall을 거절할 수 있게 했다
(`decline_agent_card_spy`, codec **v110**; 이제 모든 Spy 경로가 그렇다). 또 pytest가 평가 문제집 tips-v1의 **모든** 포지션을 복원해 본다 —
규칙을 바꿔 국면이 움직이면 그 커밋에서 곧바로 실패하므로, 같은 작업 단위에서 [evaluation/problem-set.md](evaluation/problem-set.md)의
"다시 캐는 명령"으로 다시 캔다(아래 요약). 학습·비교는 새 규칙에서 다시 시작하면 된다. 그 뒤 M10은 새 규칙에서 기준선(heuristic·5081·탐색 좌석)부터 다시 잰다. 아래 UI·학습 항목은 그대로다.

**현재 위치(2026-09-27).** UI 개선 0~10단계가 끝났다([`ui-improvement-plan.md`](ui-improvement-plan.md)의
7~10단계: 실전 감사 두 번과 선택지 36판 전수 점검에서 나온 항목, 리더 카드 위의 상태, 효과 문구와 리더 능력의
한국어 — 워크트리 `ui-work`를 master `08b5003`로 병합, 아래 요약). 학습 쪽 현재 위치는 아래 0번(M10)이 그대로다. UI에서 남은 것은 아래이며, **사용자가 고르기
전에는 시작하지 않는다**.
...
2. **아직 영어인 낱말.** Wild card·Immediate·Usurp는 용어집 행이 먼저다(한국어 룰북이나 한글판 카드
   `[KO card: …]` 인용). Other Memories·Secret Project·Fedaykin Maneuver·Into the Fray·Feyd는 리더 카드 팝오버가
   이미 한글판 인쇄 이름(다른 기억들·비밀 프로젝트·페다이킨의 책략·전투 속으로·페이드 토큰)을 쓰지만 행동 목록·로그
   라벨(`labels.js`)은 영어라, 둘을 맞출지는 사용자 결정이다. 바꾸면 `test_i18n.py`와 `log_words.py`의 허용
   목록에서도 뺀다.
3. ~~한글 카드 스캔~~ — **2026-09-23 적용**(아래 "2026-09-23 한글 카드 이름·그림"과 "혈통·불멸 한글 카드"
   세션 요약): 봉기 138·혈통 90·불멸 58장이 `cards/ko/`에, 한국어 화면은 한글 그림·한글 이름(인쇄 확인분
   251개). 남은 것은 빠진 카드 62장 — 봉기 책략 33·혈통 책략 12·뒤틀린 책략 11·교전 2(Secure Imperial
   Basin, Test of Loyalty)·대모 제시카·프로모 2 — 이고, 58장은 한글 이름도 없다. 2026-09-27 사용자가 가진
...
- **(b) 비용 줄이기 — 완료(2026-09-23)**: 판당 **405초 → 18초(약 22배)**, 강도는 잡음 안([evaluation/m10-2026-09-22.md](evaluation/m10-2026-09-22.md) 13절).
  - **합법 행만 읽기(`45dc9a3`)** — 시간의 65%가 네트워크, 그 대부분이 32,987행 정책 head였다. 탐색은 몸통을 지난 뒤 제시된
    행동의 head 행만 읽는다. 12절 확인 셀과 같은 100판이 순위·VP까지 100/100 같고 판당 405 → 76초. 병목은 이제 엔진이다
    (`apply`·`legal_actions` 56%) — 플레이아웃 배치 추론은 최대 약 20%라 미뤘다.
  - **순환 방지(`c50614f`)** — 탐색 좌석이 `switch_graft_card`를 영원히 반복해 한 판이 1시간+ 멈췄다(switch 플레이아웃이 상한에서
    잘리고 value head가 그 턴 한가운데 상태를 높게 침). `NetworkAgent`와 같은 가드(같은 라운드·관측·합법 목록에서 이미 둔 수 제외)를
    실제 결정과 플레이아웃마다 넣었다. **무변화가 아니다**: 같은 100판 중 38판이 달라지고 판당 71 → 46초, 강도는 +1%p [−3, +5].
    12절의 58%와 13절 앞쪽 변형 표는 가드 전 값이다.
  - **효과 순서는 네트워크에 맡김(`430d1ae`, 지금 `search:` 기본)** — census에서 Agent 턴 효과 순서 결정이 탐색 시간의 63%인데
    뒤집는 비율은 10%로 가장 낮았다. 새 seed 300판을 같은 버전(가드 적용)끼리 짝지으니 출하 좌석 **61.7%** 대 전체 탐색 64.3%,
    차이 −2.7%p [−10.0, +5.0], 순위 +0.06 [−0.08, +0.20], 판당 18초 대 45초. 전체 탐색은 `search_effect_order=True`.
    판정은 frame 종류(`decision_stack[-1].kind == AGENT_EFFECTS`)로 한다.
  - 도구: 셀 두 개를 seed·좌석으로 짝짓는 `scripts/ab/cross_paired.py`; census·프로파일·멈춘 판 찾기·한 수씩 추적 스크립트는
    `checkpoints/2026-09-22/search/tools/`(git 무시), 셀 원자료는 `.../search/cost/`.
  - 덤으로 드러난 것: 관측 인코딩이 Graft 짝 중 어느 카드가 앞인지 구분하지 못한다(switch 전후 관측이 같다). 네트워크가 두 상태를
    가를 수 없으므로, 관측을 다시 손볼 때 후보.
- **밤샘 실행(2026-09-24 03:07~09:11) — (c) M10 표 완료, 말단 척도 결함 수정**([evaluation/m10-2026-09-22.md](evaluation/m10-2026-09-22.md) 14절;
  원자료·판정 규칙 `checkpoints/2026-09-24/night/`). 약 4,900판에서 멈춤·실패 0.
  - **M10 표**(탐색 좌석 1명 대 상대 3명, seed 80000~ = 고정 평가 대역 v1, 귀무 25%): random 100%, heuristic **97.7%**,
    rollout **88.0%**, 2081(이전 champion) **64.0%**, 5081 **57.0%**(1,400판) — 모든 하한이 25% 위. 탐욕 5081 자체는 heuristic
    90.0%, 2081 33.7%. 탐색은 실제 상대가 제 상대 모델(5081)과 다른 2081일 때도 +30%p를 더한다.
  - **말단 척도 결함(`294d143`)**: 끝난 플레이아웃을 순위 사다리(100/70/40/10)로, 끝나지 않은 것을 value head(+1 / −1/3)로
    읽어서 게임을 끝내는 수가 4등이라도 과대평가됐다. 끝난 판을 head의 보상으로 읽으니 새 seed 1,400판 짝 비교에서 **+5.2%p
    [+3.4, +7.1]**, 순위 −0.06, 마진 +0.21(미리 정한 규칙 통과). 출하 좌석은 같은 판에서 게임을 더 일찍 끝낸 쌍이 170 대 50이었다.
    어제 seed 300판에서는 −0.3%p라 대가는 대역마다 다르다. M10 표의 행은 수정 **전** 좌석 값이다.
  - 밤에 쓴 도구(스크래치): 마감·프로세스 그룹 종료 실행기 `deadline.py`, 대기열 `queue.sh`(청크별 행 수로 완료 판정, 마감 시각 절단,
    STOP 파일), 요약 `summarize.py`, 메커니즘 `mechanism.py`.
- **Expert iteration v1(2026-09-24 낮) — 강해지지 않음**([evaluation/m10-2026-09-22.md](evaluation/m10-2026-09-22.md) 15절;
  코드 `training/expert.py`·`training/distill.py`·`cli/expert.py`, 원자료·드라이버·선언 규칙 `checkpoints/2026-09-24/exit/`).
  탐욕 5081 자가대국의 결정 절반에 탐색이 후보 값을 라벨로 달고(1,392판, 라벨 약 18만), 사전을 기울인 목표로 5081에서 학습했다.
  it1·it2가 오프라인 관문은 통과했지만 대 5081 ×3 **25.2%·24.4%**(각 1,400판, 귀무 25%)로 강해지지 않았고, 선명한 라벨만 hard로
  가르친 후보는 **20.9%**로 약해졌다. 가치 head만 재적합해 탐색에 쓰는 것도 순위를 나쁘게 했다. 네트워크가 "탐색이 뒤집는 상태"를
  비슷한 상태와 가르지 못한다(선명한 뒤집기의 18.5%만 배움) — 표현력이 다음 병목이다.
- **탐색 기본은 후보 5 × 세계 8(2026-09-25 새벽)**(16절): 손잡이 비교(각 400판, 같은 판의 `search:5081`과 짝)에서 후보 5개
  +9.5%p, 세계 8개 +5.8%p, 후보 8개는 5개와 같음, 지평 2라운드는 비용 3.4배에 이득 없음. 새 판 1,000판(80100~80349) 확인에서 후보 5개
  66.1%(대 후보 3 × 세계 4 +4.1%p [+0.1, +8.1]), **후보 5 × 세계 8 76.1%(대 후보 5개 +10.0%p [+6.4, +13.7], 대 출하 좌석 +14.1%p)**.
  `DEFAULT_CANDIDATES = 5`, `DEFAULT_ROLLOUTS = 8`, 판당 18 → 58초(결정당 약 0.27초). 새 좌석의 M10 표: random 100%, heuristic 99.3%,
  rollout 89.5%, 2081 81.3%, 5081 76.1%. **탐색끼리 2:2에서도 옛 좌석을 60% 대 40%로 이긴다**(+20%p [+11, +29], 400판).
  세계 16은 새 판 1,000판에서 세계 8보다 +2.2%p [−1.6, +5.8](규칙 미달, 합계 1,400판 +3.0%p [+0.0, +6.1])에 비용 2배 — 기본은 세계 8.
- **다음 후보(사용자 결정)**: (1) 새 기본 좌석의 M10 표·후보 5 × 세계 16 시험 결과 반영(`checkpoints/2026-09-24/exit/knobs/queue4.sh`가
  돌렸다), (2) 탐색 좌석으로 M10 표 전체를 1,400판 셀로 굳히기, (3) 표현력 — 행동·카드 임베딩이나 분해된 policy head(15절의 병목), 그 위에서 expert iteration 재시도, (4) 탐색끼리 2:2.
  M10의 "모델"에 탐색 래퍼를 포함할지는 사용자 결정이다. 남은 옛 후보: 처음부터 다른 seed, 구매 전문가 B1.
- **판정 비용 메모**: 출하 `search:` 좌석의 1대3 셀은 4 worker로 100판에 약 8분, 1,400판에 약 1.8시간이다. 전체 탐색
  (`search_effect_order=True`)은 약 2.5배.
- **논의 중(2026-09-22, 코드 변경 없음): 사용자의 실전 팁을 학습에 쓰는 법** — [player-tips-for-training.md](player-tips-for-training.md).
  ...
  **평가 문제집 틀(같은 날)**: `evaluation/problem_set.py` + `dune-imperium-problems mine|score|check` + 문제집
  `evaluation/problem_sets/tips-v1.json`(110개, 3문항) — [evaluation/problem-set.md](evaluation/problem-set.md). 포지션은 seed와
  선택 번호로 저장해 엔진만으로 복원하고, 규칙이 바뀌어 포지션이 움직이면 복원이 "re-mine" 오류를 낸다(**규칙을 고친 뒤에는
  `dune-imperium-problems check`**). 5081은 세 문항에서 1.00·1.00·0.85.
...
0. (**2026-09-26 주의 — 규칙이 다시 넓게 바뀌었다**: 카드·보드 전수 감사의 규칙 수정 약 130건(The Spice Must Flow·Covert Operation·Subversive Advisor 등 기본판 카드 포함), codec v108(저녁 판정 뒤 v109). 5081을 포함한 모든 체크포인트·대전 표·탐색 비교·문제집 채점은 옛 규칙의 것이다. 형식 2 체크포인트는 v109로 이관된다(5081: 유지 32,980·새 26·삭제 7).)
   (**2026-09-20 주의 — 환경의 규칙이 바뀌었다**: Influence 4 보너스(Emperor Spy·Guild 3 Solari)·Spy 배치 의무·Branching Path·Imperial Privilege·c7r3, codec v107. 아래의 승률·대전 수치와 1650·2050 체크포인트는 전부 옛 규칙의 것이다. 형식 2 체크포인트는 v107로 이관되지만 **이어 학습하기 전에 새 규칙에서 다시 잰다**; 세션 요약 "보드 조각·아이콘 전사 정정"의 "M10에 미치는 영향".)
...
# 사람 팁 census와 비교 (docs/player-tips-for-training.md 7절)
uv run python scripts/ab/tip_census.py --agents heuristic --games 100 --ruleset choam --bloodlines --tech-module --immortality --promo-cards --out ab-runs/tips/h
uv run python scripts/ab/tip_compare.py ab-runs/tips/h ab-runs/tips/other

# 평가 문제집 (docs/evaluation/problem-set.md); 규칙을 고친 뒤에는 check부터 (pytest의 복원 가드와 같은 검사;
# 깨지면 problem-set.md "구조"의 "다시 캐는 명령"으로 다시 캔다)
uv run dune-imperium-problems check
uv run dune-imperium-problems score --agents heuristic,checkpoint:<path>
...
다른 세션이 학습·규칙 작업을 하는 동안 워크트리 `ui-work`에서 했다. 자세한 항목과 커밋은
[`ui-improvement-plan.md`](ui-improvement-plan.md) 7~10단계.

- **7단계**(실전 감사 6관점): 로그가 긴 묶음 뒤에도 게임을 따라감, Intrigue 패스를 조용한 줄로(엔진 id), 원격 새 게임은
  좌석 넷이 사람, 반쪽 창의 머리글 한 줄, 전투 결과 한 줄(`combat_reward_gained` 이벤트에 `victory_points` — 규칙
  동작은 그대로, 이벤트 payload만).
- **8단계**(2차 감사 + 선택지 36판 전수 점검): 원격 로그는 내 마지막 행동 이후 전부 강조, 잇단 패스 한 장으로 접기,
  음모 선택지 줄에 선택지 문구, 같은 이름 Trash 후보에 구역 표시, 한국어 화면의 영어 네 군데, 저장 삭제는 두 번,
  도움말은 "한 턴 진행"부터, 새로고침 오류 문구는 회복되면 지움.
- **9단계**(리더 카드 위의 상태): Feyd·Chani 트랙 토큰(인쇄 칸 좌표 `display/leader_layout.py`), 이르쿤의 운항 카드
  네 칸(주인만 뒷면 카드를 봄 — 서버가 이미 가리던 것을 그리기만), 코타의 Secret Project 타일과 샤담의 Sardaukar
  계약을 리더 카드 아래로(시장 칸의 Sardaukar 줄은 뺌). 카드 줄 머리 "Agent:"·"On discard:"는 아이콘이 아니라 글자.
- **10단계**(한국어): 엔진 데이터로 짓는 효과 문구 — 카드·책략·운항·계약·교전·공간·기술 타일·선택 줄 설명 — 에
  한국어 쌍둥이(`*_ko`, 카탈로그 `text_ko` 등, 행동 `detail_ko`), 리더 능력은 한글판 인쇄를 두 번 옮겨 대조한 전사
  (`display/leaders_ko.py`, [`implementation-audits/leaders-ko.md`](implementation-audits/leaders-ko.md)). 언어 정책이
  바뀌었다: 효과 문구는 언어 설정을 따른다(용어집·[`rules/korean-card-style.md`](rules/korean-card-style.md)).
  용어집에 룰북 인용 행들과 새 출처 종류 `[KO card: …]`.
- **병합**: master가 그 사이 213커밋 움직여(카드면 전사 정정 등) 영어 생성기가 바뀐 곳마다 한국어 쌍둥이를 맞췄다
  (Spy 효과 이름 변경, Tread in Darkness·Long Reach 등의 문구, 아이콘 조건·접합·차례 시작 줄, Reclaimed Forces). 전사 중
  찾은 Chani·Fenring 인장 반지와 엔진의 불일치는 master가 이미 `27ed643`에서 고쳐, 병합 뒤 두 언어가 같은 말을 한다.
- **남긴 것**: 위 "UI 다음 후보" 1~3. 한 서브에이전트가 세션 기록을 읽고 사용자의 다른 질문 답("1·2·3")을 엉뚱한
  항목에 적용한 일이 있어, 워크플로 지시문에는 "이 지시만 보고 판단하라"를 넣는다.

## 2026-09-27 새벽 Agent box Spy의 거절·tips-v1 복원 가드 세션 요약 (Mac mini, 워크트리 `trusting-keller-3ad243`의 `agent-card-spy-decline` → `tips-v1-restore-guard` → master `ef943c5`, 관측 v20, codec v109→**v110** — **규칙 변경 하나, 관측 인코딩 무변경**)

- **규칙**(OQ-057 (14)): supply가 빈 채 Agent box의 Spy(Bene Gesserit Operative·Reliable Informant·Double Agent, discard를 낸 Arrakis
  Observer의 Deep Cover Spy)를 해결할 때 선행 recall을 거절할 수 있다 — "supply가 비었을 때의 선행 recall(…)은 그대로 선택이므로, 그때는
  배치하지 않고 넘어갈 수 있다"([rules/uprising-systems.md](rules/uprising-systems.md) "Spy와 observation post"), 그리고 Agent box와 공간
  효과는 "원하는 순서로 처리한다" `[Main p. 9]`. 2026-09-26의 첫 수정(`2bd566f`)은 이 box를 turn 종료에 불발하는 box로 보아
  `finish_agent_turn`으로만 넘기게 해서, 그 사이 Plot의 Recall Spy 비용(Special Mission)이 supply를 채우면 넘기기로 한 배치가 다시 의무가
  됐다. 이제 `decline_agent_card_spy`를 recall 앞에 제시하고(그 카드를 위한 recall 전까지만), 거절하면 Agent turn이 계속되며, box에 답하기
  전에는 turn 종료가 없다(Espionage의 `resolve_espionage_without_spy`와 같다). recall한 뒤에는 배치가 의무이고, recall할 Spy가 없으면
  OQ-057 (1)대로 turn 끝에 불발한다. 이로써 Spy를 놓는 모든 경로 10곳이 거절을 제시한다(목록은
  [implementation-audits/spies.md](implementation-audits/spies.md)). heuristic은 이름 접두사로 점수를 매겨 recall(1.0)을 거절(−2.0)보다
  높게 치므로 두는 방식이 그대로다.
- **codec**: v110, 모든 카탈로그 +1 — 기본 4,425·CHOAM 4,716·CHOAM+Bloodlines 11,253·전 확장 33,007. 5081 이관(v107 → v110): 유지
  32,980·새 27·삭제 7.
- **복원 가드**: pytest가 문제집의 문항마다 앞의 3개만 복원해 봐서, master `60f8e95`에서 tips-v1 106개 중 4개가 조용히 깨져 있었다(첫
  번째 `s42/p2`는 OQ-069 병합 `1420936`부터). 이제 `tests/unit/test_problem_set.py::test_every_position_of_the_committed_suite_restores`가
  모든 포지션을 `check`와 같은 함수(`problem_set.unrestorable`)로 복원하고(약 10초, 표시 없이 기본 스위트 — tip census와 같은 관례),
  깨지면 움직인 id 목록과 재채굴 안내로 실패한다. 파일의 `note`는 명령줄이 아니어서 정확한 재채굴 명령을
  [evaluation/problem-set.md](evaluation/problem-set.md) "구조" 절에 적었다(적힌 그대로 zsh에서 돌리면 커밋된 파일과 바이트 동일).
  **규칙을 바꾼 커밋에서 이 가드가 깨지면 같은 작업 단위에서 다시 캔다.**
- **다시 고정한 것**: tips-v1 — 브랜치에서 106 → 109, 병합 뒤 OQ-070 쪽 재채굴(111)과 겹쳐 다시 캐서 **111**(master의 OQ-070 파일과 같은
  국면; 7개는 거절이 recall 앞에 끼어 선택 번호만 밀림, 채점 불변). golden 인코딩·tip census·E2E seed는 그대로 통과했다.
- **리뷰**: 복원 가드 diff를 workflow(관점 셋 × 반박자 셋)로 리뷰해 확정 결함 0. 사실이 맞는 지적 둘(`note`가 명령이 아님, OQ 판정
  요약과 전사 감사 문서의 "문제집 … 그대로 복원된다")은 반영했다.
- **남긴 것**: 포지션이 움직인 것이 아니라 엔진 버그로 ValueError가 나면 가드는 그 항목을 id 없이 나열한다(예전 CLI `check`와 같은 동작).
- **검증**(병합한 트리): pytest 2,379개, Ruff, mypy; sweep(heuristic·random × 전 확장 800·기본 600·Bloodlines+draft 300, soundness
  25 간격) 3,400판 실패 0; codec 탐침(9 구성 × random 40판, 모든 합법 행동 인코딩) 빠진 행동 0 — 아홉 구성 모두에서
  `decline_agent_card_spy`가 실제로 제시됐다; 브라우저 E2E 5종 통과(`log_words` 13·`turn_end` 67·`staged_turn` 42·`lang` 38·`board_tokens` 199).

## 2026-09-27 새벽 OQ-070 Commander 배치 몫 세션 요약 (Mac mini, 워크트리 `commander-deploy-slots` → master `56f75c8`, 관측 v20, codec v109 — **규칙 변경, codec·관측 인코딩 무변경**)

- **판정**(OQ-070, 사용자): "commander 소집했으면 커맨더를 배치해야지, troop이 그 배치 몫을 차지하면 안 되지". 배치 한도를 유닛 종류별로
  센다 — 이번 turn에 recruit한 troop 수 R_t, Commander 수 R_c, 이번 turn에 배치한 troop t·Commander c에 대해 max(0, t−R_t) + max(0, c−R_c)가
  garrison 추가분(보통 2, Elite Forces 등으로 바뀜)을 넘지 않는다 `[Main p. 10]` `[FAQ p. 4]` `[Bloodlines pp. 4-5, 12]`. Agent turn의 Combat
  칸, Agent turn·Reveal의 Combat 아이콘 모두. frame 문맥에 `commanders_recruited`(Agent·배치 전 TURN)와 `reveal_commanders_recruited`(Reveal)가
  생겼고, 배치 전 turn과 Reveal 전 turn의 이월, 닫힌 turn 표시(OQ-044 (d))도 같이 맞췄다. 리뷰가 짚은 기존 결함 하나도 고쳤다: 이 turn에
  배치 몫을 세는 중에 이전 turn의 유닛이 retreat하면 종류 구분 없이 빼던 계산(`reconcile_deployment_after_retreat`).
- **방식**: 세션 모델로 구현 → 리뷰어 둘(규칙·집계 / 테스트·회귀) 1라운드 승인 → 경미한 지적 반영. 리뷰어는 모든 배치·회수 순서를 전수 탐색했고
  116판 fuzz에서 제시된 최대치를 식과 30,907번 대조해 어긋남 0이었다.
- **다시 고정한 것**: golden 인코딩 둘(promo_bloodlines_tech·everything; 옛·새 인코더가 모든 벡터에서 같음), tips-v1 재채굴(106 → **111**, 채점은
  [evaluation/problem-set.md](evaluation/problem-set.md)), tip census 고정 판 셋(Gruesome Sacrifice → full seed 24 seat 2, Influence 두 역할 →
  seed 177 seat 1·seed 161 seat 0), E2E `log_words.py` seed 38 → 47.
- **검증**: master pytest 2,374개, Ruff, mypy; 브랜치에서 sweep 3,400판 실패 0, codec 탐침 빠진 행동 0, E2E 7종; 병합 뒤 master sweep 300판.
- 참고: 병합 메시지 `56f75c8`의 "Also pins the forced Spy move order"는 틀렸다 — 그 고정은 이미 master 직접 커밋(`6fb8816`·`60f8e95`)이었다.

## 2026-09-26 저녁 OQ 판정 세션 요약 (Mac mini, 워크트리 `oq066-*`·`oq068-*`·`oq069-*` → `oq-integration` → master, 관측 v20, codec v108→**v109**)

감사 뒤 OPEN이던 다섯 판정 가운데 셋을 사용자가 정했다(인용은 [rules/open-questions.md](rules/open-questions.md), 기록은
[감사 문서](implementation-audits/transcription-audit-2026-09-26.md) "사용자 판정 2차").

- **OQ-066**: Reclaimed Forces의 acquire도 acquire → face-up Call to Arms가 발동한다. 함께: Call to Arms의 troop이 어느 획득 경로에서든
  Reveal의 recruit로 세지 않던 결함.
- **OQ-068**: 모든 Agent recall(Steersman, Twisted Mentat, Sardaukar II·High Council contract 보상, CHOAM Demands 경로)이 Imperial
  Privilege처럼 Conflict의 Into the Fray Agent를 되돌린다. 새 행동 `recall_conflict_agent_for_agent_card`(Bloodlines)·
  `recall_conflict_agent_for_contract`(CHOAM+Bloodlines) → codec v109(전 확장 33,006; 5081 이관 유지 32,980·새 26·삭제 7). 커밋 7afb22b는
  단독으로는 깨진 중간 상태다(bisect 때 건너뛴다).
- **OQ-069 (A)**: Maker Hooks가 있으면 Desert Power의 2 Persuasion은 Persuasion 갈래를 고를 때까지 쓸 수도 Command (6+)에 셀 수도 없고,
  고르기 전에는 Reveal을 끝낼 수 없다. 오전의 "2를 쓰면 sandworm을 닫는다"는 없어졌다.
- **OQ-064·065**: 사용자가 사실 확인을 물었고(contract bank 3장 미만일 때만 / Research Station·Spice Refinery에 Spy 12개가 다 있을 때만)
  판정은 아직이다.
- **방식**: 판정마다 구현 → 독립 반박 리뷰 → 수정 루프를 워크트리에서 돌렸다(구현 Sonnet, 리뷰 세션 모델; 약 2시간). 사용자는 이 뒤
  CLAUDE.md의 workflow 모델 정책을 지우고 subagent 모델 선택을 main 세션에 맡겼다(`baddb50`).
- **검증**: master pytest 2,304개, Ruff, mypy; 통합 브랜치에서 sweep 2,200판(heuristic·random × 전 확장 800·Bloodlines+draft 300) 실패 0,
  codec 탐침 빠진 행동 0, 브라우저 E2E 7종 통과. golden 인코딩·문제집·census 고정 판은 그대로 복원된다(이 판정들이 닿는 판이 드묾). (2026-09-27 정정: 문제집은 아니었다 —
  OQ-069 병합 `1420936`부터 `s42/p2` 하나가 복원되지 않았는데, pytest가 문항마다 앞의 3개만 복원해 봐서 놓쳤다. 지금은 복원 가드가 전부 본다.)
- **recruit 집계 전수 수정**(`recruit-count-sweep`, 밤에 병합 `82b1d80`): "그 turn에 어떤 출처에서 recruit했든 새 troop은 Conflict에
  deploy할 수 있다" `[Main p. 10]` `[FAQ p. 4]`를 모든 recruit·trash 지점에 맞췄다 — Arrakis Revolt·Occupation의 획득 troop, Eliminate
  Allies의 모든 trash 경로(겹친 optional trash 창 포함), Plot·Reveal의 recruit, Reveal 중 Commander, Reveal 전 turn의 recruit 이월. 그리고
  turn이 닫힌 뒤 열리는 후속 효과(optional trash, 대기 중인 Skill 선택·Navigation play, Usurp)는 `turn_closed`로 표시해 다음 turn 몫에
  넣지 않는다(OQ-044 (d)); `GameState.pending_skill_choices`·`pending_navigation_plays`의 항목에 넷째 필드가 생겼다(관측 인코딩 무변경).
  리뷰 5라운드, 마지막 두 리뷰어 승인. 검증: master pytest 2,350개, sweep 3,400판 실패 0, codec 탐침 빠진 행동 0, E2E 7종.
- **OQ-064**(같은 병합): Coercive Negotiation은 contract bank에 3장 미만이면 hand에서 낼 수 없고 face up이어도 발동하지 않는다 — 사용자
  판정 "계약은 리필되는 수단이 없으니 3장이 없으면 ... 아예 사용을 못 하는거지".
- **OQ-065**(master 직접 커밋 `79f5eed`·`6fb8816`·`60f8e95`): False Orders·Holy War의 강제 Spy 이동은 카드를 쓴 좌석의 다음 좌석부터
  시계 방향으로, 한 좌석은 옮길 Spy를 자기 차례에 모두 옮긴다(OQ-036 (b), `[FAQ p. 3]` Mohiam 선례). Agent 공간과 연결되지 않은 빈 post가
  없으면 그 Spy는 supply로 돌아간다 — 사용자 판정 "갈 곳 없는 스파이는 공급처로 되돌아가게 하기"(Rival 규칙 `[Bloodlines p. 8]`를 따른
  convention의 확정).
- **자기 trash 경로의 recruit guard**(워크트리 `gallant-jepsen-9d6f86`, 커밋 `e118a1c`·`3404b29`): recruit 집계 전수 수정의 "낡은 frame 문맥
  덮어쓰기"(`lessons.md` 2026-09-26 항목 4)와 "Reveal 중 아무 것도 세지 않음"의 두 모양이 카드가 자기 자신을 trash하는 9곳에 남아 있었다.
  "trash될 때: troop" 트리거(`RECRUIT_TWO_TROOPS`)는 Eliminate Allies뿐이고 그 카드에는 이 9곳의 자기 trash 효과가 없어 지금 판에는
  영향이 없다 — **규칙·codec·관측 인코딩 무변경**. (Eliminate Allies가 자기 box로 자기 자신을 trash하는 것은 가능하고 이미 맞게 센다 —
  병합 전 검토가 짚어 box 경로 테스트를 하나 더했다.) Agent box 5곳(Seek Allies, Subversive Advisor,
  Treacherous Maneuver의 자기 trash, Dangerous Rhetoric의 `trash_self` 아이콘, Scientific Breakthrough)에 `keep_trash_recruits`, Reveal 쪽
  4곳(Bombast의 Command trash 세 경로, Engineered Miracle)에 `credit_trash_recruits`. 테스트 9개는 trash 트리거를 가짜로 붙여
  (`card_trash._trash_effect` monkeypatch) guard를 하나씩 빼면 그 테스트만 실패함을 확인했다(`implementation-audits/bloodlines.md`
  Eliminate Allies 행 (g)). 검증 중 낡은 `.pyc`가 결과를 뒤섞은 일을 `lessons.md`에 적었다.

## 2026-09-26 카드·보드 전사 전수 감사 세션 요약 (Mac mini, 워크트리 `fix-s1`~`s8`·`fix2-r1`~`r3` → `fix-integration` → master, 관측 v20, codec v107→**v108** — **규칙 변경 130여 건, 관측 인코딩 무변경**)

2026-09-25 Chani·Fenring Signet 오독 뒤 사용자가 "보드 칸, 모든 카드, 책략, 기술, 리더 능력" 전수 검사를 요청했다. 기록은
[implementation-audits/transcription-audit-2026-09-26.md](implementation-audits/transcription-audit-2026-09-26.md)(방법·발견 114건과
처리·통합 리뷰 18건·사용자 판정·새 OQ), 교훈은 [lessons.md](lessons.md) 2026-09-26.

- **방법**: 348항목(시작 8·Reserve 2·Imperium 109·Tleilaxu 20·Intrigue 90·Contract 28·Conflict 18·Tech 18·Skill 7·Leader 19·보드 칸 23·
  보드 항목 6)을 엔진을 보지 않은 블라인드 전사 → 엔진 데이터·코드·표시 문구 대조 → 불일치마다 검증자 3명의 반박 시도로 확인했고,
  BGG 카드 인벤토리와 기계 대조를 따로 돌렸다(감사가 놓친 것 없음, 시트 오류 8건은 그림으로 판정). 확정 **114건**(규칙 80·표시 32·
  불확실 2) 중 113건을 여덟 작업 단위(카드 데이터, 새 Imperium 효과, Agent box, Reveal, Intrigue, Spy·Conflict·Contract,
  Tech·Skill·Leader·Ixian Embassy, 표시 문구)로 고쳤다. #29(Subversive Advisor가 먼저 trash될 때의 기본 Influence)는 공식 문서가
  답하지 않는 드문 순서라 두었다.
- **기본판 카드도 틀려 있었다**: The Spice Must Flow의 Reveal(검 1 → spice 1), Covert Operation의 Reveal(Persuasion 2 → Spy 2),
  Subversive Advisor(Solari 1 → Persuasion 1), Calculus of Power·Maker Keeper·Undercover Asset·Chani, Clever Tactician의 Agent 아이콘,
  Prepare the Way·Bene Gesserit Operative·In High Places·Overthrow의 소속, Sardaukar Coordination의 Persuasion 2. 2026-08부터 모든
  판·학습이 옛 값으로 돌았다.
- **사용자 판정**: Tenuous Bond·Grasp Arrakis·Battlefield Research는 칸의 색띠와 카드 아래의 인쇄(COMBAT / ENDGAME, PLOT / COMBAT)대로
  칸마다 한 시점만; Coercive Negotiation은 조건이 되면 반드시 수행(거절 없음); False Orders는 FAQ대로 Agent 공간에 **연결되지 않은** 빈
  post로 옮기고 Holy War에도 같은 판정(OQ-036 (b)).
- **통합 리뷰**: 병합 중의 교차 충돌 넷(Hardy의 `reveal_water`, Commander 배치 템플릿의 카탈로그, 비공개 정보 섞기와 공개 contract, Servo
  신호 id의 로그 누출)을 고친 뒤 여섯 영역 독립 리뷰의 확정 18건을 세 단위(`fix2-r1-reveal`·`r2-leaders`·`r3-agent-spy-intrigue`)로
  고쳤다. main 세션이 후속 둘을 더 고쳤다 — Agent box trash·Long Live the Fighters로 trash한 Eliminate Allies의 troop 2, Rapid
  Engineering·Battlefield Research로 산 tile의 troop이 그 turn의 배치 몫에서 빠지던 것(`[Main p. 10]` `[FAQ p. 4]`).
- **codec·관측**: `ACTION_CODEC_VERSION` **108**(새 거절·회수·선택 행동, 시점 정리). 카탈로그 기본 4,439 → 4,424, CHOAM 4,729 → 4,715,
  CHOAM+Bloodlines 11,228 → 11,250, 학습 구성(전 확장) 32,987 → 33,004. `OBSERVATION_VERSION` 20 유지 — 인코더 파일은 바이트 동일,
  `FrameKind.LEADER_SIGNET`은 끝에 붙어 기존 인덱스 불변, golden 5판에서 옛·새 인코더가 모든 벡터에서 같다.
- **M10에 미치는 영향**: 형식 2 체크포인트는 v108로 이관된다(5081: 유지 32,980·새 24·삭제 7). 규칙이 기본판 카드까지 넓게 바뀌어
  **기존 체크포인트·대전 표·탐색 비교·문제집 채점은 모두 옛 규칙의 것이다.** 다음 학습·비교는 새 규칙에서 기준선부터 다시 잰다.
- **다시 고정한 것**: golden 인코딩(두 번, 인코더 동일성 확인 뒤), 평가 문제집 tips-v1(104 → 108 → **106**, 채점은
  [evaluation/problem-set.md](evaluation/problem-set.md)), tip census 고정 판, 서버 seed, `turn_end` seed 7, `scripts/e2e/log_words.py`
  seed 30 → 38.
- **검증**: pytest 2,279개, Ruff, mypy; sweep(heuristic·random × 전 확장 800·기본 600·Bloodlines+draft 300, soundness 25 간격)
  3,400판 실패 0; codec 탐침(9 구성 × random 40판, 모든 합법 행동 인코딩) 빠진 행동 0; 브라우저 E2E 7종 통과.
- **OPEN으로 남은 판정 다섯**(현재 동작을 적은 채 사용자 판정 대기): OQ-064 Coercive Negotiation이 가져갈 contract가 없을 때,
  OQ-065 강제 Spy 이동에 갈 곳이 없을 때, OQ-066 Reclaimed Forces의 "acquire"와 Call to Arms, OQ-068 Recall Agent와 Into the Fray
  Agent, OQ-069 Desert Power의 선택 전 2 Persuasion과 Command (6+)(현재는 sandworm을 골라도 이미 받은 Command 효과가 남는다).
- **커밋 메시지 정정**(기존 커밋은 고치지 않는다): `e797452`의 "13 of 107 positions moved"는 20개, `b6fa486` 마지막 항목의
  "base seed 3 seat 2"는 full seed 3 seat 2.

## 2026-09-25 Bloodlines 카드면 전사 정정 세션 요약 (Mac mini, master 직접 커밋, 관측 v20, codec v107 — **규칙 변경 셋, codec·관측 인코딩 무변경**)

사용자가 한글 Leader 카드를 전사하다 찾은 두 곳(Chani, Fenring)과, 그 뒤 Deep Cover 아이콘 전수 검색으로 찾은 한 곳(Storms in
the South)이다. 영문·한글 카드면이 같고, 공식 Bloodlines 룰북(영·한)과 FAQ에는 두 Signet에 대한 clarification이 없다(p. 12의
Fenring 항목은 Intrigue trash에 Solari가 없다는 것뿐). 커밋 `27ed643`(Chani·Fenring: 코드·테스트·표시·문제집), `e797452`(Storms in the
South: 코드·테스트·문제집·census 고정 판 — 메시지의 "13 of 107 positions moved"는 잘못 적은 것이고 `check`는 87/107, 즉 20개였다), 문서 커밋. 교훈은 [lessons.md](lessons.md) 2026-09-25.

- **Chani — Fedaykin Maneuver**: 유료 절반 "[Fremen] 2 Influence: [water] → [초록 카드][초록 카드]"의 초록 카드는 draw 아이콘이다
  (`assets/icons/draw.png`, Y'rkoon의 Hungry for Spice와 같은 그림). 2026-09-07부터 troop 2를 recruit하고 있었다 → 이제 water 1을
  내고 카드 2장을 draw한다(`draw_or_request_personal_cards`; 덱이 모자라면 discard reshuffle chance). `troops_recruited`는 늘지 않는다.
- **Count Hasimir Fenring — Corrino Liaison**: Spy 아이콘이 회색 원기둥 뒤에 금색 원기둥이 겹친 **Spy with Deep Cover**(Deliver
  Supplies contract와 같은 그림; 같은 확장의 Mohiam Listeners는 회색 원기둥 하나)다. `rules/bloodlines.md` 4절 "일반 규칙대로 Spy
  하나를 놓되, 놓을 때 상대의 Spy를 무시할 수 있다. 자신의 Spy가 이미 있는 post에는 놓을 수 없다. `[Bloodlines pp. 5, 12]`" → Emperor
  post(`emperor-sardaukar-dutiful-service`, 하나뿐)에 상대 Spy가 있어도 놓을 수 있고, 자기 Spy가 있으면 못 놓는다
  (`_leader_spy_placement_actions(..., deep_cover=True)`; supply가 비면 회수 먼저는 그대로).
- 나머지 Bloodlines Leader 7장의 능력·Signet 칸도 아이콘 단위로 다시 대조했고 전사와 일치했다.
- **Deep Cover 아이콘 전수 검색**(2026-09-19 교훈의 절차): Deliver Supplies에서 자른 아이콘을 Uprising·Bloodlines·Immortality 카드
  368장에 배경 마스크 다중 스케일 템플릿 매칭(짧은 변 360px, `TM_CCOEFF_NORMED`; 스크립트는 스크래치)했다. 진짜 일치 5장(0.89~0.98)과
  나머지(0.80 이하)가 뚜렷이 갈렸다: Deliver Supplies, Spy Drones, Fenring, Arrakis Observer, 그리고 **Storms in the South** — 엔진
  목록(앞의 넷)에 없던 하나다.
- **Storms in the South**(Bloodlines Conflict II): 1등 보상이 Spy with Deep Cover + spice 2인데 일반 Spy로 전사돼 있었다
  (`implementation-audits/bloodlines.md`의 Conflict 카드 행). `ConflictReward.deep_cover_spies`(계약의 `ContractReward`와 같은 이름)를
  더하고, `combat._spy_placement_frame(..., deep_cover=True)`가 frame 문맥에 `deep_cover`를 실어 `legal_combat_reward_spy_actions`가
  자기 Spy가 없는 모든 post를 연다(일반 보상 Spy frame의 문맥은 그대로라 다른 Conflict의 replay는 바뀌지 않는다). 새 프롬프트
  "Choose an Observation Post for your Spy with Deep Cover"의 한국어를 `prompts_ko.js`에 더했다.
- **codec·관측**: 행동 id(`pay_leader_signet_water`, `place_leader_spy(post_id)`, `place_combat_reward_spy(post_id)`)와 템플릿이 그대로라 `ACTION_CODEC_VERSION` 107,
  `OBSERVATION_VERSION` 20 유지. 체크포인트는 이관 없이 그대로 로드된다.
- **M10에 미치는 영향**: 환경 규칙이 세 곳 바뀌었다. 학습 구성의 Leader 18명 회전에서 4인 판의 약 40%(1 − C(16,4)/C(18,4))에
  Chani나 Fenring이 앉지만, 결과가 달라지는 것은 그 Signet을 쓸 때 — Chani는 water 지불을 고를 때, Fenring은 상대 Spy가 Emperor
  post에 있을 때 — 뿐이다. Storms in the South는 Bloodlines 판의 Conflict II 풀에 들어가 자주 나오고(문제집 재채굴에서 20개 국면이
  움직인 것이 그 흔적), 1등이 상대 Spy가 있는 post를 고를 수 있게 됐다. 기존 체크포인트(5081 등)는 Chani의 `pay_leader_signet_water`를 "troop 2"로 배운 값을 갖고 있다: 다음 학습은
  고친 환경에서 그대로 이어 가면 되고(이관 불필요), 옛 대전 표·탐색 비교 수치는 이 세 규칙이 닿는 판에서 조금 다를 수 있다.
  heuristic은 행동 id 점수표라 이 정정으로 수가 바뀌지 않는다(Chani의 water 행동은 표에 없어 기본 점수).
- **평가 문제집 재채굴 두 번**: 정정마다 복원이 깨져(`dune-imperium-problems check` 96/110, 이어서 87/107) 파일의 `note` 명령 그대로
  다시 캤다(heuristic seed 0–799·800–2799, 5081 seed 0–59; Mac mini `--workers 8`로 약 2분). 110 → 107 → **104개**(sandworm 80,
  마지막 라운드 보유 17, Endgame 7). 바뀌거나 빠진 국면은 첫째에서는 전부 Chani·Fenring 판, 둘째에서는 전부 Storms in the South가
  덱에 든 판이었다(무관한 판이 새로 든 것은 문항당 40개 상한의 꼬리 보충뿐). 채점은 [evaluation/problem-set.md](evaluation/problem-set.md).
- **tip census 고정 판 이동**: Storms 정정이 full 룰셋 seed 3·23의 판을 바꿨다 — 옛·새 엔진으로 같은 판을 두어 보니 둘 다 처음 갈라지는
  곳이 Storms in the South의 1등 보상 Spy다. (a) `test_tip_census_endgame.py`의 Panopticon 회귀 사례(Endgame을 여는 Tech 효과가
  선두를 바꿈)는 seed 23에서 사라져, seed 1–120 중 그런 판이 유일한 **seed 45**(Mohiam이 Panopticon으로 VP 9 → 11, seat 0을 넘음)로
  옮겼다. (b) `test_tip_census_influence.py`의 손으로 추적한 seed 3 seat 1 고정값은 새 경로에서 여러 열이 0이 돼 검출력이 떨어지므로,
  seed 3–199에서 그 열들을 함께 덮는 두 좌석(seed 158 seat 0, seed 156 seat 0)으로 나눠 고정했다. 새 값은 수정하지 않은 collector의
  출력이고, Influence 합계·출처 합·Spy 배치/trash·Infiltrate 수를 그 판의 이벤트와 대조했다(손 추적은 아니다).
- **UI**: 영문 Leader 표시(`display/leaders.py`)와 행동 라벨(`labels.js`·`labels_en.js`의 `pay_leader_signet_water`)을 "카드 2장
  draw"로, Fenring의 Signet 문구를 "Spy with Deep Cover"로, Conflict 보상 문구(`display/structs.py`)에 "with Deep Cover"를 더했다.
  떠 있는 8000번 서버는 재시작해야 새 규칙과 문구가 적용된다.
- **검증**: pytest 1,975개 통과(새 회귀 테스트 `test_fedaykin_maneuver_retreats_any_number_or_draws_two_cards`·
  `test_fedaykin_maneuver_draw_shuffles_the_discard_when_the_deck_is_short`·`test_corrino_liaison_spy_has_deep_cover`·
  `test_storms_in_the_south_first_place_spy_has_deep_cover`는 옛 코드에서 실패함을 확인), Ruff, mypy, 마지막 트리의 sweep(`--ruleset both --bloodlines --tech-module --immortality --promo-cards --rotate-leaders --soundness-interval 25`) heuristic 800판·random 800판 완주, 실패 0.

## 2026-09-24 되돌리기로 숨겨진 카드가 보이던 구멍 세션 요약 (Mac mini, master 직접 커밋, 관측 v20, codec v107, 변경은 `core/observation.py`의 `known_card_seats`·`simulation/invariants.py`·테스트·`docs/` — **엔진 규칙·코덱·관측 인코딩 무변경**)

- **사용자 지적**(실전): Tleilaxu Row의 카드를 산 뒤 "되돌리기"가 됐고, 그 사이 Row 보충으로 Tleilaxu deck의 다음 카드를 보게 됐다.
- **원인**: 되돌리기 경계(OQ-010, `open-questions.md`: "숨겨진 더미에서 누군가에게 흘러간 정보(자기 draw 포함)는 되돌릴 수
  없고")는 `known_card_seats`로 카드의 공개 여부를 판정하는데, 그 목록에 확장의 뒷면 더미가 없었다 — Tleilaxu deck
  `[Immortality p. 4]`, Skill stack `[Bloodlines p. 3]`, Tech stack의 맨 위 아래 `[Bloodlines p. 6]`. 목록에 없는 카드는 "모두 공개"로
  읽혀, 보충·뒤집기로 드러나도 공개로 잡히지 않았다. 수정 전 코드에서 사람 좌석의 `acquire_tleilaxu`·`acquire_tech`는
  `undoable: true`였다(재현 확인).
- **전수 대조**: 무작위 판의 모든 상태에서 "각 좌석의 관측 + 그 좌석에게 놓인 선택지(합법 행동 인자) + 좌석 한정 이벤트"가
  보여 주는 카드와 `known_card_seats`를 비교하는 검사기로 전 확장 200판(상태 144,660개, 카드 판정 4,100만 건)을 훑었다. 더
  찾은 것: 선택지로 소유자에게만 보이는데 목록이 "아무도 모름"으로 둔 카드 — Controlled·Glowglobes의 덱 맨 위, Long Live the
  Fighters의 위 3장, Coercive Negotiation이 공개하는 계약 3장, Kota Odax의 맨 아래 Tech 타일(과 Secret Project). 이 선택지를 연
  스텝은 되돌릴 수 있었다(본 것을 안고 다시 고르기). 쓰이지 않은 Conflict 카드도 넣었다(드러날 일은 없지만 목록의 정확성).
  사용 중인 Navigation 카드는 해결 중인 Intrigue처럼 공개로 맞췄다.
- **수정**: 위 전부를 `known_card_seats`에 넣었고, 공개 이벤트 스윕 불변식의 **따로 적힌** 숨겨진 존 목록(`_hidden_instances`,
  Tech stack·Secret Project는 여기에만 있었다)을 `known_card_seats` 하나로 합쳤다. 관측(`observe_state`)과 학습 관측 인코딩은
  그대로다.
- **검증**: pytest **1,950개 통과**(+12: `tests/unit/test_known_card_seats.py` 8 — 존별 5 + 세 룰셋 판의 전수 대조 3, 약 3초;
  `tests/server/test_undo.py` 3 — Tleilaxu·Tech 구매가 되돌릴 수 없음, 확장 더미의 공개 판정), 새 테스트 11개 모두 수정 전
  코드에서 실패 확인, ruff·mypy 통과, 브라우저 E2E `staged_turn.py`·`recovery.py`·`turn_end.py` 녹색. 검사기의 남은 불일치 1건은
  Coercive Negotiation이 공개하지만 Intrigue가 없어 가질 수 없는 계약이 선택지에서 빠지는 경우로, 목록 쪽이 맞다.

## 2026-09-23 밤 ~ 09-24 새벽 턴 종료 한 번 누르기 통일 세션 요약 (Mac mini, master 직접 커밋, 관측 v20, codec v107, 변경은 `server/turn_end.py`(새)·`server/sessions.py`·`server/persistence.py`·`server/static/`·`scripts/e2e/`·`tests/server/`·`docs/` — **엔진·코덱·학습 코드 무변경**)

- **사용자 지적**: 턴 끝이 일관되지 않다 — 마지막 리더 픽 등에서는 "턴 종료"를 누를 틈 없이 넘어가고, 어떤 때는 선택지의
  "Agent 턴 종료"를 누른 뒤 "턴 종료 확정"을 또 묻는다(사용자: "이전에도 지적했다").
- **원인**(조사 워크플로 8 에이전트 + 6판 census): 2026-09-03의 멈춤 조건이 "다음 결정이 다른 좌석의 PlayerDecision이고 그
  좌석의 되돌리기 창이 열려 있을 때"였다. 그래서 (1) 마지막 스텝이 카드를 뽑았거나(창 0) 다음이 chance면 누를 틈 없이 넘어갔고,
  (2) 마지막 리더 픽은 First Player가 하므로 다음 결정이 자기 1라운드 턴이라 hand-over 자체가 없었고, (3) `finish_agent_turn`·
  `finish_reveal`·`pass_*_intrigue` 뒤에는 창이 열려 있어 두 번째 확정이 붙었고(2026-09-05 "이중 확인이지만 유지"), (4) 내 효과로
  상대가 버리기를 고를 때·남의 끼어들기에 답할 때는 턴이 끝나지 않았는데 "턴 종료 확정"을 물었다. 6판 표본에서 사람 좌석
  hand-over 216번이 0·1·2번 누름으로 흩어져 있었다.
- **규칙(프로젝트 UI 관례, 규칙서 밖)**: 사람 좌석의 **모든 턴 끝은 그 좌석이 정확히 한 번 누른다.**
  - 엔진의 명시적 종료 행동 `finish_agent_turn`·`finish_reveal`·`pass_combat_intrigue`·`pass_endgame_intrigue`는 그 한 번 자체다:
    서버가 봉인(`undo_floor`)하고 곧바로 넘긴다. 목록에는 `undoable: false`로 나간다.
  - 그 밖의 스텝이 좌석의 **단위**(Player Turns의 Agent/Reveal 턴, 그 밖 단계의 자기 몫 — 리더 픽, Combat/Endgame Intrigue 차례,
    보상, Control 방어)를 끝내면 서버가 `confirmation`으로 멈춘다 — 되돌릴 것이 남았든 아니든. 판정은 세션의 `open_units`(좌석 →
    누른 뒤 그 좌석이 자기 단위 안에서 한 첫 스텝의 번호)다: 누름(확정 또는 명시적 종료)이 닫고, 되돌리기가 그 첫 스텝을 물리면
    닫히며, 턴 시작에서 턴을 치르기 전에 한 일(Plot Intrigue, Tech flip)은 열지 않는다(`agent_placed`·`turn_passed`·
    `turn_start_card_played`가 턴을 치른 표시). 열린 단위는 다음 결정이 다른 좌석의 단위가 되거나 그 좌석이 **자기 턴의 시작**
    (`turn` frame이 맨 위)에 서면 끝난다 — 마지막 리더 픽 → 자기 1라운드 턴, 보상·Control 방어 → 다음 라운드의 자기 턴, 다른
    좌석이 모두 Reveal한 뒤의 연속 턴. 같은 좌석의 다른 결정은 단계가 바뀌어도 이어진다(Conflict 승리로 발동한 Intrigue 선택이
    Makers가 시작된 뒤에 온다).
  - 첫 구현은 `turn_marks`(마지막으로 턴 시작에 선 시점)와 라운드 비교로 "새 턴"을 판정했는데, 리뷰 워크플로(3관점, 재현
    스크립트 포함)가 세 결함을 재현했다: Holy War류 뒤 다음 좌석이 아직 치르지 않은 턴 시작에서 멈춤, 다른 좌석의 시작된 턴 안에서
    열리는 `intrigue_trigger_spy`가 턴 중간 멈춤·이중 누름을 만듦, 불러온 판에서 Control 방어 뒤 멈춤이 빠짐. `open_units`와
    "시작된 Agent/Reveal 턴 위의 다른 좌석 frame은 모두 응답"(`turn_end.unit_seat`)으로 바꿔 셋 다 사라졌다.
  - 다른 좌석의 응답은 끝이 아니다 — 시작된 Agent/Reveal 턴 위의 다른 좌석 frame 전부, 그리고 어디서든 끼어들기 frame 셋
    (`opponent_card_discard`·`opponent_spy_move`·`opponent_unit_loss`): 답한 좌석은 멈추지 않고, 내 효과로 상대가 답하는 동안에도 멈추지 않는다. AI의 답이 사람의 턴을 닫으면 그 사람에게 멈춘다(행동한 좌석이 아닌
    좌석의 멈춤). 알려진 예외: Holy War류가 마지막 효과면 엔진이 다음 좌석의 턴 frame을 먼저 열고 그 위에 상대의 결정을 쌓으므로,
    누름이 상대의 답보다 먼저 온다(여전히 한 번).
  - 스텝 직후의 chance는 멈춤 판정 전에 푼다(턴 중간의 reshuffle은 끝이 아니고, 턴이 chance 안에서 끝날 수도 있다).
  - 멈춘 좌석은 다음 결정이 자기 것이어도 합법 행동이 없고(`legal_actions` 빈 목록, snapshot `actions: null`) 행동하면 거부된다.
  - 저장 문서에 `confirmation`·`sealed_steps`·`open_units`를 적는다(형식 버전은 그대로, 선택 필드). 불러오면 그대로 돌아오므로 사람 → 사람으로
    넘긴 직후의 자동 저장이 불러온 뒤 다시 묻던 것(2026-09-17 slice 5에서 알고 남긴 동작)과 그 좌석의 되돌리기 창이 다시 열리던
    것이 사라졌다. 그 필드가 없는 옛 문서는 옛 규칙으로 복원한다.
- **화면**: "턴 종료 ▶" 버튼 하나가 늘 같은 자리(배너, 안내문 아래·되돌리기 위)에 같은 모양으로 선다. 멈춤이면 `confirmTurn()`,
  명시적 종료 행동이 합법이면 그 행동(목록에서는 빠진다). 라벨: 기본 "턴 종료 ▶", 이번 Reveal에서 산 것이 있으면
  "구매 끝 · 턴 종료 ▶"(예전처럼 아직 살 수 있는 카드가 있을 때), 패스는 "패스 · 턴 종료 ▶". 대기 문구 "…의 턴 종료를 기다리는 중", 알림·도움말 문구도 같은 말로.
- **검증**: pytest **1,935개 통과**(+26, `tests/server/test_turn_end.py`; `test_undo.py`의 옛 규칙 테스트 둘은 새 규칙으로 고쳐 씀),
  ruff·mypy 통과, 브라우저 E2E **16종 녹색**(`rehearsal.py` 제외 전부; 새 `turn_end.py` 67항목 — 마지막/중간 리더 픽,
  `finish_agent_turn`·`finish_reveal`·`pass_combat_intrigue`·`pass_endgame_intrigue`가 배너 버튼이고 목록에 없음, 되돌릴 수
  없는 턴 끝의 멈춤, 영어 라벨; 옛 클라이언트에서 실패함을 리뷰가 확인). 새 테스트는 리뷰가 지적한 여섯 변이(AI 응답의 멈춤 끄기,
  명시적 종료의 봉인 빼기, 복원의 봉인 무시, 명시적 종료의 자동 저장 빼기, 턴 시작 예외 빼기, 시작된 턴 판정 빼기)에서 각각
  실패함을 확인했다. 스크래치 적대적 검사(사람 1~4좌석·random/heuristic·전 확장·리더 드래프트·되돌리기·저장/복원 쌍둥이)
  270판에서 "누름 없이 턴 두 번", "한 턴에 두 번 누름", "누른 뒤 자기 스텝 없이 멈춤", "단위 중간 멈춤", "멈춘 좌석의 행동",
  "명시적 종료 뒤 멈춤", "복원 쌍둥이 불일치" 0건.
- **남긴 것**: (1) Holy War류 예외(위). (2) **기존 결함**(이번 변경과 무관, `HEAD`에서도 재현): "한 단계 되돌리기"를 연달아 두 번
  하면 세션 로그가 `A(undone), B(undone), Undo(1), Undo(1)`이 되어 그 판의 저장(자동 저장 포함)이 복원되지 않는다
  (`_rebuild_log`의 "undo marker count 1 does not match the 2 undone step(s)"). **같은 날 고쳤다**(`a19f6e6`): 기록 형식은
  맞았고(순서대로 읽으면 스택 — 스텝은 쌓이고 표지는 위에서 그 수만큼 걷어 낸다, `LoggedUndo` docstring), 읽는 쪽 둘
  (`_rebuild_log`, 검토의 `undo_history`)이 "표지 바로 앞 연속 N개"를 가정했다. 둘을 스택으로 읽게 바꿔 디스크의 옛 파일도
  그대로 읽힌다; 회귀 테스트 3개(`test_undo.py`), 되돌리기 30%·저장/복원 쌍둥이 118개 적대적 검사에서 복원 오류 0. (3) 행동 로그의 턴 카드
  묶음(`panels.js`의 `QUIET_ACTIONS`)에 있던 `"pass"`는 엔진에 없는 id였다(실제는 `pass_combat_intrigue`·`pass_endgame_intrigue`) —
  UI 7b(`8226e86`)에서 고쳤고, 8h에서 잇단 패스는 카드 한 장으로 접는다.

## 2026-09-23 혈통·불멸 한글 카드 세션 요약 (WSL 노트북 i5-8250U, master 직접 커밋, 관측 v20, codec v107, 변경은 `display/names_ko.py`·`server/static/`의 한국어 문구·테스트·`scripts/e2e/lang.py`·`docs/` — **엔진·학습 코드 무변경**; 에셋 저장소 커밋은 아래)
...
- **커버리지**: 불멸은 manifest 58장 **전부**(임페리움 25·틀레이락스 20·음모 11·실험·연구 기지 오버레이). 혈통은
  ...
  판·항해 카드 10 전부, 음모·뒤틀린 책략은 30장 중 8장(30번 사진이 더미라 온전히 보이는 것만; 급속 공학은 가림).
- **새 모양**: 기술 타일·계약·장소 타일은 영문 렌더에 정합해 그 크기로 잘랐다. **지휘관 기술 토큰은 사다리꼴**이라
  (아래 변 0.834, 높이 0.860 — 13장 측정) 737x634 PNG에 투명 바깥으로 두었다(영문 렌더는 정사각 재작업). 화면은
  `.vcard.skill` 폭 64px·높이 자동이라 그대로 맞는다. 붙어 놓인 불멸 음모 15장은 3x5 격자 칸을 영문 카드에 정합.
- **이름**: `names_ko.py`에 136개 추가(합 251 — 새 절 `skills`·`tech`). 두 번 읽어 163/164 일치, 한 건(챠니)은
  3배 확대로 판정(판정 에이전트의 "찬니/챤니"는 오독, 봉기 인쇄 "영리한 전술가, 챠니"와 같다). Skirmish (Wild)는
  한국어판 룰북의 **와일드 배틀 아이콘** `[Bloodlines p. 5]`으로 "소규모 전투 (와일드)" — 용어집에 행 추가.
  "실험체 X-137"은 카드에 라틴 문자가 인쇄돼 있어, 이름 테스트를 "한국어 이름의 라틴 문자는 영어 이름에도
  있어야 한다"로 넓혔다. 항해 카드는 제목이 인쇄되지 않아 이름이 없다.
- **화면 문구**: 한국어 라벨이 이제 한글 이름이 있는 카드를 영어로 적던 라벨·문구 20곳을 인쇄 이름으로 — 세포 수확·
  금지된 무기·파놉티콘·틀레이락스의 주인·공포의 누더기 괴물·되돌아온 병사들·플라스틸 칼·반중력 의복·의자개,
  프로모 옵션의 무자비한 리더십·천재적인 조언자, 파이터, 기술 모듈 옵션의 익스의 코타 오닥스, Shaddam 계약
  줄은 "사다우카 {계약} · 샤담 코리노 4세 전용". `lang.py`의 영어 허용 목록에서 지도자 약칭(Shaddam·Kota Odax)을
  뺐다. Feyd 토큰은 룰북 확인 전이라 그대로.
- **교훈 재확인**: 풀 SIFT 매처는 비슷한 기술 타일 두 장을 서로 바꿔 달았고(배송 구획↔사다우카 총사령부, 고속
  수송선↔오니솝터 부대), 격자 칸 하나는 이웃 카드(멜란지 연구)에 정합해 제목이 잘렸다 — 판독 단계가 잡았고
  올바른 렌더로 다시 정합해 잘랐다. 기하 검증(여백 합)은 봉기와 같은 방법으로 했다.
- **기준**: pytest **1,906개 통과**(수는 그대로 — 기존 이름 테스트에 새 세트 단언을 더함), ruff·mypy 통과, 브라우저
  E2E 15종 녹색(`open_mode.py`의 "1초 안에 다른 브라우저의 수가 보인다"는 큰 git 커밋과 겹친 첫 실행에서 한 번
  실패, 단독 재실행 56/56 — 시간 검사는 무거운 작업과 같이 돌리지 않는다).
...
- 이어서 사용자 요청으로 푸시(`7940040`)하고 **평가 문제집 틀**을 만들었다: `src/dune_imperium/evaluation/problem_set.py`, CLI
  `dune-imperium-problems`(pyproject에 등록 — 다른 기기에서는 `uv sync` 후 쓴다), 문제집 `tips-v1.json`, 테스트
  `tests/unit/test_problem_set.py`(17개). Opus 검증 넷이 문항 판정의 반례와 틀 결함을 찾았고 모두 반영했다(문서의 "검증에서 배운 것").
  결과와 5081의 확신 오답 두 포지션은 [evaluation/problem-set.md](evaluation/problem-set.md).
...
- **재생**(`app.js`의 `playback`·`turnStops`·`playbackTick`): 단위는 **한 턴씩**(기본)/한 수씩, 간격은 0.25·0.5·**1**·2·4초. turn stop은 행동 로그의 턴 카드와 같은 규칙(`finish_agent_turn`·`finish_reveal`·`pass_combat_intrigue`·`pass_endgame_intrigue`·`pick_leader`가 턴을 닫거나 다음 행동의 좌석이 다름)이고, chance step은 앞 행동에 붙으며, 연달은 패스(아무도 안 쓴 Combat Intrigue 창)는 한 stop이다(2026-09-24 UI 7b에서 엔진 id로 고침). 간격은 "움직임에서 움직임까지"로 잰다(응답 시간을 뺀다). 탐색(⏮·⏭·슬라이더)은 재생을 유지하고 손으로 넘기기(◀·▶·좌석 행동)는 멈춘다. 끝에 닿으면 멈추고 재생 버튼이 "처음부터 재생"이 된다.
...
- 남은 경계: 로컬 UI는 `finish_agent_turn` 뒤에도 기존 turn 종료 확인(되돌리기 창)을 한 번 더 거친다(이중 확인이지만 되돌리기 보호 목적이라 유지 — **2026-09-24에 사용자 지적으로 폐기**: 명시적 종료 행동이 곧 턴 종료의 한 번이다, 위 "턴 종료 한 번 누르기 통일" 세션 요약). 효과가 직접 배치한 troop의 회수 여부는 판정 범위 밖(필요하면 OQ-029 확장).
...
3. **턴 종료 확인 완료.** 사람 좌석의 행동 뒤 다음 결정이 다른 좌석의 것이고 그 좌석의 되돌리기 창이 열려 있으면 세션이 `awaiting_confirmation`(요약 `confirmation`)으로 멈춘다. `POST /games/{id}/confirm`(`confirm_turn`)으로 확정해야 chance·AI가 진행되고, 되돌리기는 멈춤을 해제한다. 공개(reveal)로 끝난 턴은 창이 이미 닫혀 있으므로 예전처럼 바로 넘어간다. (**2026-09-24 대체**: 이제 모든 턴 끝이 되돌릴 것이 없어도 한 번의 누름을 기다린다 — 위 "턴 종료 한 번 누르기 통일" 세션 요약.) 저장 중 멈춤 상태는 복원 시 유지된다. 서버 테스트 helper(`_play`)는 자동 확정한다.
