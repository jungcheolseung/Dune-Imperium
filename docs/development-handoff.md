...
3. ~~한글 카드 스캔~~ — **2026-09-23 적용**(아래 "2026-09-23 한글 카드 이름·그림"과 "혈통·불멸 한글 카드"
   ...
   251개). 남은 것은 빠진 카드 — 봉기 음모 33장·대모 제시카·프로모 2장, 혈통 음모 10장·뒤틀린 책략 11장 —
   이며, 그 출처 후보는 TTS 한글화 모드 3종이다.

**UI를 이어갈 때의 규칙**(세 세션이 값을 치른 것들):
- `static/*.js`를 고치면 pytest로는 부족하다. [`scripts/e2e/`](../scripts/e2e/README.md) **17종**을 돌린다(2026-09-24 `turn_end.py` 추가; `rehearsal.py`는 Tailscale 주소가 있을 때만).
...
- **남긴 것**: (1) Holy War류 예외(위). (2) **기존 결함**(이번 변경과 무관, `HEAD`에서도 재현): "한 단계 되돌리기"를 연달아 두 번
  ...
  묶음(`panels.js`의 `QUIET_ACTIONS`)에 있는 `"pass"`는 엔진에 없는 id다(실제는 `pass_combat_intrigue`·`pass_endgame_intrigue`) —
  로그 표시만의 문제라 손대지 않았다.
