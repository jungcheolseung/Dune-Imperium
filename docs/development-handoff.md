...
**진행 중(2026-09-08, 사용자 지시 "Immortality 확장 구현"): M13 Immortality.** 슬라이스 1(출처·명세·옵션 골격·board 전사·카탈로그)과 슬라이스 2(Bene Tleilax board의 상태·행동, specimen, 개정 Research Station, Experimentation, Family Atomics; OQ-048~051; 관측 v12, codec v96), 슬라이스 3(Tleilaxu Row 획득·Reclaimed Forces·획득 box의 Research/Tleilaxu·첫 Tleilaxu 카드 3장)은 완료했고, 다음은 슬라이스 4 — **Graft**: Agent turn에 카드 두 장을 함께 play하는 행동(`agent_turn`에 `graft_card_id` 인자; Graft 카드는 혼자 play 불가, 두 카드 모두 "보낸" 것, 두 Agent box가 효과 frame의 자유 순서 그룹에 함께 들어감), "if grafted"·"the other grafted card" 판정, Ghola의 box 복사, Usurp의 Imperium Row graft(turn 끝 trash), Chairdog의 Reveal 시작 반환, Tleilaxu Infiltrator의 점유 무시, Beguiling Pheromones의 grafted 카드 trash(FAQ: trash 비용 효과 동시 불가). 그 뒤 5(카드 play data — 전사는 [implementation-audits/immortality.md](implementation-audits/immortality.md)에 있다), 6(UI·heuristic·소크). 세부는 [implementation-plan.md](implementation-plan.md)의 M13 절. 공식 문서가 침묵하는 판정은 구현할 때 open-questions에 등록한다(후보는 audit 문서의 "미완 경계").
