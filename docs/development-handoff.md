...
2. **M10 강화학습과 league self-play(다음).** M9의 평가 행렬과 self-play 러너 위에서 시작한다 (`implementation-plan.md`의 M10). 첫 슬라이스 후보: (a) 학습 라이브러리 결정 — 현재 의존성에 torch가 없다(`rl` extra는 gymnasium·pettingzoo·numpy). PyTorch를 `rl` 또는 새 `train` extra로 넣을지는 사용자 결정 사항이다. (b) `BatchPolicy`를 구현하는 masked policy 네트워크(관측 v4 2,022-int 입력, codec v89 4,354 출력, 좌석 egocentric이라 가중치 공유)와 체크포인트(관측·codec 버전 기록). (c) 종료 보상 기준 REINFORCE/PPO 학습 루프 — `stack_episodes` 배치를 그대로 쓴다. (d) league: 학습 정책 + 과거 체크포인트 + `AgentBatchPolicy("heuristic"|"rollout")`를 lineup으로 섞고, 평가는 `dune-imperium-tournament`에 학습 정책을 registry 이름으로 등록해서 한다. 학습 seed와 평가 seed는 분리한다(대회 기본 policy offset 900,000, self-play 기본 1,000,000).
