...
**현재 위치(2026-09-10 심야, HEAD `88c0ef7`).** 사용자가 고른 다섯 항목 중 ①문서 정정·②검증
가드·③heuristic 보드 공간 재정비·**④처리량 35% 회귀 귀인**이 끝났고, 남은 것은
**⑤M10 학습 재개**(사용자 방침으로 최후순위)뿐이다. ④의 결론: 회귀는 실재하며 **step당 엔진
CPU가 약 1.6배**가 된 것이고, 원인은 한 곳이 아니라 확장 4종을 담게 된 엔진 전반이다. 실제 결함
2건(`ab48e0c` Protocol 검사, `88c0ef7` 확장 존 검증)을 고쳐 −4~5.5%를 회수했고, 앞 세션이 지목한
용의자 `6782c89`는 2.7%짜리 소수 항이었다. 전체 수치·배제한 설명·남은 후보 5개는
[evaluation/throughput-2026-09-10.md](evaluation/throughput-2026-09-10.md)에 있다.

그 밖에 남아 있는 두 항목: **확장 룰셋의 공간 순위**(보너스를
더하는 방식은 −34%p로 기각됐고 Combat 균형 재설계가 필요하다)와 **`agent_turn`의 남은 인자
동점 995개**(같은 공간에 어느 카드를 낼지 — 카드 가치 평가가 필요하다). 세부는 2026-09-10 밤
세션 요약과 [evaluation/baseline-2026-09-10.md](evaluation/baseline-2026-09-10.md) 13절에 있다.
...
- **결론: 회귀는 실재하고 step당 엔진 CPU가 약 1.6배가 된 것이다.** 이 저장소(WSL2, 8코어)에서
  같은 명령을 세 트리에 번갈아 재어 8 worker **1.62배**, 단일 프로세스 **1.66배**로 재현했다.
...
- **개별 기여와 크기 정정.** 8 worker 중앙값 16.50 → 16.30(Protocol만, **−1.2%**) → 15.80(둘 다,
  **−3.1%**), `ms/decision`은 0.082 → 0.044(**−46%**). 즉 **Protocol 수정은 `ms/decision` 회귀의
  사실상 전부지만 벽시계로는 소수 항**이다. 커밋 `ab48e0c`의 메시지는 이 항목을 "0.9s 중 0.62s"로
  적었는데 **과대평가**다 — 0.62초는 CPU 시간이고 8 worker 병렬이라 벽시계로는 그 1/8이다.
  커밋은 다시 쓰지 않고 정정을 evaluation 문서 3(a)와 [lessons.md](lessons.md)에 남겼다.
...
- 검증: pytest **1,480**(신규 6: tournament 불변식 1 + 모듈 게이트 4 + tech_flipped 1), Ruff, mypy
  통과. 소크: base+CHOAM random 200판과 전 확장 heuristic 120판을 `--soundness-interval 10
  --privacy-interval 5`로, 230,678 step 실패 0.
- **남은 후속**: (a) 아직 남은 성능 후보 5개는 evaluation 문서 5절에 크기 순으로 있다. 가장 안전한
  다음 수정은 base에서 항상 `()`만 돌려줄 수 있는데 O(1) 게이트가 없는 legal action provider
  6개다. `core/engine.py:112`의 재열거(결정당 legal 집합을 두 번 만든다)는 이 구간 이전부터 있던
  안전 가드라 성능이 아니라 **설계 판정**이 필요하다. (b) 확장 룰셋의 공간 순위, (c) `agent_turn`
  인자 동점 995개, (d) M10 학습 재개는 그대로 남았다.
