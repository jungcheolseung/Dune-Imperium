...
1. **M9 평가 러너와 baseline(진행 중).** 순서는 (1) 대회 도구 + 지표 — **2026-09-06 완료**(`dune-imperium-tournament`; 첫 기준선 `docs/evaluation/baseline-2026-09-06.md`), (2) **다음: rollout/search baseline** — `agents/registry.py`에 이름을 등록하면 대회 도구가 바로 평가한다; heuristic을 rollout 정책으로 쓰는 1-ply 또는 MCTS 계열을 만들고 위 기준선 행렬에서 heuristic보다 유의미하게 강해야 한다, (3) 여러 게임을 병렬 구동하고 정책 추론만 batch하는 self-play 러너 — M10의 학습 정책 추론 인터페이스가 정해진 뒤 붙인다 (`implementation-plan.md`의 M9). 관측·보상 계약은 [`rl-environment.md`](rl-environment.md)로 고정돼 있고, 병렬 실행 선례는 `simulation/sweep.py`다. 더 큰 야간 규모 재검증이 필요하면 `dune-imperium-sweep --games 50000 --ruleset both --workers 8 --rotate-leaders --soundness-interval 25 --coverage-json ...`을 커밋된 트리에서 돌린다.
