...
2026-09-08(Immortality 슬라이스 1 뒤)의 기준 결과는 pytest 1,309개 통과(카드 이미지 에셋이 없는 머신은 1,308 통과 + 1 skip; `train` extra가 없으면 추가로 10 skip), Ruff 통과, mypy 통과다. 현재 action codec은 `ACTION_CODEC_VERSION = 95`(기본 4,366개, CHOAM 4,652개, `promo_cards` 옵션 시 4,466/4,752개; `bloodlines`·`tech_module` 옵션은 별도 카탈로그로 훨씬 크고, `promo_cards`+`bloodlines`는 10,451개)이고, 관측은 `OBSERVATION_VERSION = 11`의 3,729-int 전체 게임 인코딩이다(v6~v9는 Bloodlines·Tech Module 세그먼트를 더한 것, v10은 Bloodlines 프로모 Ruthless Leadership의 identity 1개, v11은 Immortality 카탈로그의 Imperium 25·Intrigue 11 identity; 옵션을 끈 룰셋에서는 새 칸이 전부 0이지만 길이가 달라져 v8 이전 체크포인트는 거부된다) ([`rl-environment.md`](rl-environment.md)). 보드 22칸 완결 + 즉시 공개 + `fab266f`/`e6fc298` 수정 + sweep 확장(`853ecd4`) 반영 후의 교차 소크는 random 룰셋당 2,000판 + heuristic 룰셋당 1,000판(둘 다 `--rotate-leaders`) + draft 두 policy 각 룰셋당 500판, 전부 `--soundness-interval 25`를 켠 총 7,000판이 실패 0으로 통과한 상태다(2026-09-01, 아래 세션 요약. 그 전 단계에서는 random 룰셋당 3,000판 비회전 소크도 실패 0이었다).
...
**진행 중(2026-09-08, 사용자 지시 "Immortality 확장 구현"): M13 Immortality.** 슬라이스 1(출처·명세·옵션 골격·board 전사·카탈로그)은 완료했고, 다음은 슬라이스 2 — Bene Tleilax board의 상태와 행동(`PlayerState`의 research 위치·Tleilaxu 위치·specimen 수·Family Atomics, `GameState`의 Tleilaxu deck·Row·첫 도달 spice, research 전진 선택 frame과 보너스 해결, Tleilaxu track 보너스, specimen 생성·반환·지출), 개정 Research Station(`board_effects.py`의 `research_station` 분기), Experimentation 시작 덱(`starting_deck_instance_ids(player, immortality=)`), Family Atomics 행동, 관측·codec 확장. 그 뒤 슬라이스 3(Tleilaxu Row 획득·Reclaimed Forces·deck 맨 위 배치), 4(Graft), 5(카드 play data — 전사는 [implementation-audits/immortality.md](implementation-audits/immortality.md)에 이미 있다), 6(UI·heuristic·소크). 세부는 [implementation-plan.md](implementation-plan.md)의 M13 절. 공식 문서가 침묵하는 판정은 구현할 때 open-questions에 등록한다(후보는 audit 문서의 "미완 경계").
