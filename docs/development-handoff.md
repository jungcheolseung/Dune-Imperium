...
- **다음 후보(사용자 결정)**: (c) M10 완료 표(탐색 대 heuristic·`rollout`, 1,400판 셀이 이제 약 1.8시간), (d) 탐색의 선택을 학습
  목표로 쓰는 expert iteration — 정체한 학습을 탐색으로 다시 움직이는 표준 경로(결정당 0.078초면 학습 iteration 한 번의 탐색 좌석
  결정 약 6,400개가 4 worker로 약 2분). 남은 옛 후보: 표현력, 처음부터 다른 seed, 구매 전문가 B1.
