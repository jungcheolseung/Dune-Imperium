...
OQ-065(강제 Spy 이동 순서와 갈 곳이 없는 Spy)도 밤에 사용자 판정으로 확정했고, 자기 자신을 trash하는 경로 9곳에 trash-recruit guard를 예방적으로 넣었다(아래 요약). 알려진 남은 결함: 이번 turn에
recruit한 Commander의 배치 몫을 garrison의 옛 troop이 대신 쓸 수 있다(배치 한도를 유닛 종류 구분 없이 합산; Agent turn·Reveal 모두,
이전부터). 학습·비교는 이것들이 정리된 뒤 다시 시작하는 것이 좋다. 그 뒤 M10은 새 규칙에서 기준선(heuristic·5081·탐색 좌석)부터 다시 잰다. 아래 UI·학습 항목은 그대로다.
