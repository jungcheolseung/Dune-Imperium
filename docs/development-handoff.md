...
모두 정해졌다(OPEN 없음). 학습·비교는 새 규칙에서 다시 시작하면 된다. 그 뒤 M10은 새 규칙에서 기준선(heuristic·5081·탐색 좌석)부터 다시 잰다. 아래 UI·학습 항목은 그대로다.
...
# 평가 문제집 (docs/evaluation/problem-set.md); 규칙을 고친 뒤에는 check부터
...
- **검증**: master pytest 2,304개, Ruff, mypy; 통합 브랜치에서 sweep 2,200판(heuristic·random × 전 확장 800·Bloodlines+draft 300) 실패 0,
  codec 탐침 빠진 행동 0, 브라우저 E2E 7종 통과. golden 인코딩·문제집·census 고정 판은 그대로 복원된다(이 판정들이 닿는 판이 드묾).
