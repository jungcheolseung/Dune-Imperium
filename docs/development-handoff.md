...
- **다음 후보(사용자 결정)**: (d) 탐색의 선택을 학습 목표로 쓰는 expert iteration — 정체한 학습을 탐색으로 다시 움직이는 표준 경로.
  교사(탐색)의 말단 결함이 고쳐졌으므로 이제 라벨을 모을 수 있다(결정당 0.078초면 학습 iteration 한 번의 탐색 좌석 결정 약
  6,400개가 4 worker로 약 2분). 그 밖에: 수정 후 좌석으로 M10 표 다시 재기, 탐색끼리 2:2, 탐색 노브(세계 8·후보 5·말단 무덱) 거르기.
  M10의 "모델"에 탐색 래퍼를 포함할지는 사용자 결정이다. 남은 옛 후보: 표현력, 처음부터 다른 seed, 구매 전문가 B1.
