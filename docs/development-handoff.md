...
- **정책 유도 탐색 — 성공(2026-09-23)**: 결정화는 그대로 두고 후보·플레이아웃·말단 평가를 네트워크로 바꾼 스크래치 변형
  ...
  ([evaluation/m10-2026-09-22.md](evaluation/m10-2026-09-22.md) 12절). **다음 후보(사용자 결정)**: (a) `src/` 정식 편입 +
  서버 AI 좌석에 연결(사람 상대로 바로 쓸 수 있다), (b) 비용 줄이기(큰 결정만 탐색, 플레이아웃 배치), (c) M10 완료 표(탐색 대
  heuristic·`rollout`), (d) 탐색의 선택을 학습 목표로 쓰는 expert iteration — 정체한 학습을 탐색으로 다시 움직이는 표준 경로.
  남은 옛 후보: 표현력, 처음부터 다른 seed, 구매 전문가 B1.
