...
- **(b) 비용 줄이기 — 1단계 완료(2026-09-23)**([evaluation/m10-2026-09-22.md](evaluation/m10-2026-09-22.md) 13절):
  - **합법 행만 읽기(`45dc9a3`)** — 시간의 65%가 네트워크, 그 대부분이 32,987행 정책 head였다. 탐색은 이제 몸통을 지난 뒤
    제시된 행동의 head 행만 읽는다. **12절 확인 셀과 같은 100판이 순위·VP까지 100/100 같고**, 판당 405 → 76초, 결정당
    2.0 → 0.37초, 100판 3시간 → 35분. 이제 병목은 엔진(`apply`·`legal_actions` 56%)이라 플레이아웃 배치 추론은 최대 약 20%다.
  - **census**(1,197결정, `checkpoints/2026-09-22/search/tools/search_census.py`): 탐색은 22.6%를 뒤집고, **Agent 턴 효과 순서**
    결정이 시간의 63%인데 뒤집는 비율은 10%로 가장 낮다. 네트워크 확신도 필터는 소용없다(99.9% 확신에서도 23% 뒤집음).
  - **싼 변형**(같은 seed 100판, 전체 탐색과 짝, `scripts/ab/cross_paired.py`): 효과 순서 결정만 탐욕인 `search_noeff`가
    판당 **23초(3.3배 더 쌈)에 55%**, 승률 차 −3 [−14, +8], 순위 차 +0.01 — 가장 가깝다. 세계 2개 56%(−2), 세계 1개 45%
    (순위·마진 손해 분해), 둘을 합친 것 44%(승률 차 분해). 변형은 스크래치(`checkpoints/2026-09-22/search/tools/cheapsearch.py`,
    `PYTHONPATH`와 `DUNE_PROBE_CKPT`; 실행 스크립트 `.../search/cost/variants.sh <이름>`).
  - **다음 결정(사용자)**: `search_noeff`를 `src/`의 기본으로 삼을지. 100판은 "큰 손실 없음"까지만이라, 더 넓은 seed로
    짝 비교(전체와 `search_noeff`를 새 seed 300판씩, 약 2시간)가 판단 근거가 된다. 편입한다면 판정은 프롬프트 문자열이 아니라
    `state.decision_stack[-1].kind == FrameKind.AGENT_EFFECTS`로 한다(4판 3,055결정에서 정확히 일치).
...
- **판정 비용 메모**: 탐색 1좌석 1대3 셀은 이제 4 worker로 100판에 35분(`search:`), `search_noeff`면 약 10분이다. 1,400판
  표준 셀은 각각 약 7.4시간·2.2시간이다.
