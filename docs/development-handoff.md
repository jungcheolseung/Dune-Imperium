...
0. (2026-09-07 완료) **M12 Bloodlines 슬라이스 1~5** — 아래 세션 요약. 다음은 슬라이스 6 Tech Module(Ixian Embassy board, Tech tile 18장의 획득·할인·Flip·acquire 효과·능력, Tech 전용 Imperium 2장(Ixian Ambassador ×2)·Intrigue 2장(Battlefield Research, Rapid Engineering), Kota Odax of Ix)과 슬라이스 7(서버·UI의 Bloodlines 표시 마감, heuristic agent의 새 행동 가중치, `--bloodlines` 대규모 소크, 관측 v8 기준 학습 재개). Tech tile 18장의 전사는 에셋 저장소 `cards/en/bloodlines/tech/`와 `other/Ixian Embassy Board.avif`에서 한다.
...
| Bloodlines 구성물 | `content/bloodlines/sardaukar.py`(Commander·Skill), `content/uprising/imperium.py`·`intrigue.py`의 `bloodlines_only`/`twisted`/`navigation` 항목, `rules/sardaukar.py`, `rules/spy_moves.py`·`rules/unit_loss.py`(상대 결정 frame), `rules/optional_trash.py` |
...
2026-09-07: `bloodlines` 브랜치(35 커밋)를 master 쪽에서 `--no-ff`로 머지했고(`dbd9b73`) 이 문서를 그때 한 번 갱신했다. 아직 push하지 않았다면 `git log origin/master..master`로 확인한다. 비공개 에셋 저장소(`assets` symlink → `Dune-Imperium-assets`)에도 같은 날 manifest 커밋 5건(Bloodlines 카드 44장 content id, Leader 8종, Tuek's Sietch 타일 이미지, Twisted·Navigation 카드 키)이 있으니 다른 머신에서는 그쪽도 pull한다.
