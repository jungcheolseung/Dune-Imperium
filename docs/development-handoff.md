...
**진행 중(2026-09-08, 사용자 지시 "Immortality 확장 구현"): M13 Immortality.** 슬라이스 1(출처·명세·옵션 골격·board 전사·카탈로그)과 슬라이스 2(Bene Tleilax board의 상태·행동, specimen, 개정 Research Station, Experimentation, Family Atomics; OQ-048~051; 관측 v12, codec v96)는 완료했고, 다음은 슬라이스 3 — Tleilaxu Row 획득(Reveal turn의 `acquire_tleilaxu`: specimen 지불, discard 또는 첫 genetic marker 뒤 deck 맨 위 선택, Row 보충)과 Reclaimed Forces의 선택 효과(`acquire_reclaimed_forces(choice)`), Harvest Cells용 Combat 중 획득 진입점. 그 뒤 4(Graft), 5(카드 play data — 전사는 [implementation-audits/immortality.md](implementation-audits/immortality.md)에 이미 있다; Tleilaxu 카드 하나라도 play data가 생기면 setup의 `setup:tleilaxu_deck` 셔플이 chance stream에 들어온다), 6(UI·heuristic·소크). 세부는 [implementation-plan.md](implementation-plan.md)의 M13 절. 공식 문서가 침묵하는 판정은 구현할 때 open-questions에 등록한다(후보는 audit 문서의 "미완 경계").
