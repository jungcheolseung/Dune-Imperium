...
### 보드 이미지 기반 한 화면 테이블 후속

- 사용자 제공 `images/map.jpg`(저장소 제외)를 `/game-images/map.jpg`로 로컬
  서빙한다. `DUNE_IMPERIUM_GAME_IMAGE_DIR`로 다른 디렉터리를 지정할 수 있다.
- 게임 화면은 viewport 고정 테이블이다. 중앙 보드의 22개 공간에 좌표 기반
  hotspot을 두고 합법 공간은 발광하며, 배치 Agent는 좌석색 토큰으로 표시한다.
- Imperium Row는 보드 하단, 4인 공개 상태는 좌측, 내 손패는 화면 하단에
  고정한다. 카드 캐시가 있는 항목은 실제 카드 이미지를 사용한다.
- 기존 텍스트 공간 목록과 손패 렌더러는 호환성 참고용으로 남아 있으나 새
  `render()` 경로에서는 테이블 렌더러를 사용한다.
