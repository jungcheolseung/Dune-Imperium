...
1. **M9 평가 러너와 baseline(진행 중).** 순서는 (1) 대회 도구 + 지표 — **2026-09-06 완료**(`dune-imperium-tournament`; 기준선 `docs/evaluation/baseline-2026-09-06.md`), (2) rollout/search baseline — **2026-09-06 완료**(`RolloutAgent`, registry 이름 `rollout`; heuristic 3명 상대 48% 승률, 결정당 약 25ms). 남은 것: (3) 여러 게임을 병렬 구동하고 정책 추론만 batch하는 self-play 러너 — M10의 학습 정책 추론 인터페이스가 정해진 뒤 붙인다. 선택 과제(필요할 때): rollout의 강화(후보·determinization 수, 2라운드 horizon, 값 함수 가중치 조정)와 M11 서버 AI 좌석에 `rollout` 추가(서버 `sessions.py`는 아직 `choose_action`만 호출하므로 `StateAgent` 경로를 연결해야 한다) (`implementation-plan.md`의 M9).
