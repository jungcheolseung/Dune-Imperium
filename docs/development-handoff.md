...
## 2026-09-09 Bloodlines contract token 세션 요약 (master, 관측 v16, codec v98)
...
- 검증: pytest 1,430(+12: 새 `test_bloodlines_contracts.py` 10건, manifest 2건), Ruff, mypy 통과. 첫 전체 실행에서 heuristic Bloodlines 게임 2건이 "Contract population changed"로 실패해(trash 대기 중 token이 어느 영역에도 없었음) token을 frame이 열린 동안 active 영역에 두는 방식으로 고쳤다.
