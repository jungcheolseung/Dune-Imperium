...
- **남은 자의 조각**: 재실행 귀무 분포(같은 설정·다른 seed 3팔 × 200 iteration, 조용한 기계에서 약 2.8시간). 이것이 없으면 어떤 A/B 임계값도 임의값이다. 사용자 결정으로 이번 세션에서는 띄우지 않았다.
