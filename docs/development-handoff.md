...
- **탐색 후보 5개가 기본(2026-09-24 저녁)**(16절): 손잡이 비교(각 400판, 같은 판의 `search:5081`과 짝)에서 세계 8개 +5.8%p,
  **후보 5개 +9.5%p**, 지평 2라운드는 비용 3.4배에 이득 없음. 새 판 1,000판 확인에서 후보 5개 66.1% 대 62.0%, **+4.1%p [+0.1, +8.1]**
  (선언한 규칙 통과), 합계 1,400판 +5.6%p [+2.4, +8.9]. `DEFAULT_CANDIDATES = 5`, 판당 18 → 31초.
- **다음 후보(사용자 결정)**: (1) 후보 8개·후보 5 × 세계 8 결과 반영(`knobs/queue2.sh`가 돌렸다), (2) 수정 후 탐색 좌석으로 M10 표
  다시 재기, (3) 표현력 — 행동·카드 임베딩이나 분해된 policy head(15절의 병목), 그 위에서 expert iteration 재시도, (4) 탐색끼리 2:2.
