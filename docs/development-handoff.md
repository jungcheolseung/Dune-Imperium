...
- **남긴 것**: (1) Holy War류 예외(위). (2) **기존 결함**(이번 변경과 무관, `HEAD`에서도 재현): "한 단계 되돌리기"를 연달아 두 번
  ...
  (`_rebuild_log`의 "undo marker count 1 does not match the 2 undone step(s)"). 별도 작업으로 넘겼다. (3) 행동 로그의 턴 카드
