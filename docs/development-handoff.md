...
**OQ-064·065는 사용자가 사실 확인을 물은 상태로 판정을 기다린다.** 진행 중: recruit 집계 전수 수정(브랜치·워크트리 `recruit-count-sweep`,
아직 master 밖 — 리뷰 통과 뒤 병합). 학습·비교는 이 둘이 끝난 뒤 다시 시작하는 것이 좋다. 그 뒤 M10은 새 규칙에서 기준선(heuristic·5081·탐색 좌석)부터 다시 잰다. 아래 UI·학습 항목은 그대로다.
...
- **진행 중**: recruit 집계 전수 수정(`recruit-count-sweep`) — "그 turn에 recruit한 troop은 배치 몫에 든다" `[Main p. 10]` `[FAQ p. 4]`를
  모든 recruit·trash 지점에 맞추는 작업(Arrakis Revolt·Occupation의 획득 troop, 겹친 optional trash 창, Reveal 중 Commander, 닫힌 turn의
  몫 등). 리뷰가 회귀를 하나 잡아 수정 라운드 중이다.
