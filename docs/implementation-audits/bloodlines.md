...
| Reveal turn | `begin_reveal_turn`: Self-Destroying Messages Persuasion 1(Command 합계에 포함, OQ-043), Delivery Bay Solari 2·Training Depot 검 2는 Persuasion ≥ 6일 때 즉시(`tech_granted` 문맥 키), 늦게 6에 닿으면 `grant_late_reveal_effects`가 지급; Panopticon은 troop 1을 Reveal recruit로 더하고, Spy 배치와 Forbidden Weapons의 선택은 Reveal frame의 문맥 `tech_reveal_pending`에 남아 REVEAL frame 행동(`place_tech_spy` → `spy_placement` frame; `choose_tech_strength[(faction[, alliance_recipient])]`/`choose_tech_trash`)으로 소유자가 원하는 시점에 해결한다. `legal_finish_reveal_actions`는 Forbidden Weapons가 남았거나 Panopticon의 Spy를 아직 놓을 수 있으면 종료를 막고, 놓을 Spy가 없으면 종료 시 `tech_reveal_unavailable`로 소멸한다. | OQ-043·OQ-044(사용자 지적으로 시작 시 강제에서 자유 순서로 재판정). |
