...
| Spy reward | One frame opens per printed Spy, times the sandworm multiplier; each selects an empty Observation Post (a Spy with Deep Cover: any post without the owner's own Spy [Bloodlines pp. 5, 12]) and spends one Spy from supply. | Placement is mandatory while a Spy is in supply (OQ-057 (14)). With the supply empty the frame offers `decline_combat_reward_spy` and `recall_spy_for_combat_reward` per own post, "you may first recall one of your Spies for no effect" [Main pp. 11, 20]; after a recall only the placement, the post just left included. Only a player with no Spy in supply and none on the board has the frame dropped (`combat_reward_spy_unavailable`). Corrected 2026-09-26: the frames used to be capped at the supply at payment, so a winner with all three Spies placed lost the Spy. |
...
| Influence | Friendship VP, printed track bonuses, Alliance acquisition or upward transfer, and Influence loss use shared transitions. A choose-Influence reward (Skirmish (Crysknife), Spice Freighters, Propaganda's "Choose two") offers every Faction below the top of its track, "Choose any one of the four Factions" [Main p. 20]; Bene Gesserit at 3 stays a choice with the Intrigue deck empty, the Influence 4 card being drawn after the discard is reshuffled [FAQ p. 2] (corrected 2026-09-26; an older guard hid it). | Captured Mentat proves that a legal loss reaches multiple recipients only when they were tied with the holder; the holder explicitly selects one as required by the FAQ. |
