...
| Conditions | `InfluenceAtLeast`, `HasHighCouncil`, `SpiesPlacedAtLeast` |
| Costs | `PayResources` |
| Rewards | `GainResources`, `GainVictoryPoints`, `RecruitTroops`, `DrawPersonalCards`, `DrawIntrigueCards`, `GainCombatStrength` (transcribed, Combat play not wired yet) |

An option is playable when at least one section applies and the sum of every
applicable cost is affordable. Playing the card pays all applicable costs and
resolves the applicable rewards in printed order; draws are requested last so a
reshuffle chance frame sits on top of the stack.
...
Remaining Intrigue identities (31) have setup identity only and are not
offered for play.
