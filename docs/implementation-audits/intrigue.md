...
| Harvest accounting | Spice paid for an Intrigue during the Agent-turn frame is added to `spice_spent_after_placement`; Spice gained from an Intrigue raises `spice_at_placement`. | Harvest Spice Contracts only count Spice harvested at the space, so Intrigue trades neither hide nor fake a harvest. |
