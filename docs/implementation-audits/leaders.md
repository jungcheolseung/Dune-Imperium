...
- **Tactician** — "Whenever you retreat or lose any number of troops from the Conflict, advance your Tactics token that many spaces, earning rewards as you reach them. Reset the token after reaching the end of the track." 11칸 track: 4인은 3번째 칸(index 2)에서 시작, 6번째 칸 spice 1, 마지막 칸 water 1 뒤 시작 칸으로 reset; 끝을 넘기는 초과분은 버린다 `[Bloodlines p. 12]`. `rules/tactics.py`가 leaf `units.retreat_units` 안에서 호출되므로 모든 retreat 경로(Reveal·Intrigue·Signet)와 Conflict에서의 유닛 손실(`unit_loss`, Holy War)이 자동으로 센다. Commander는 troop으로 센다 `[Bloodlines p. 4]`. FAQ: "When resolving combat, troops that return to your supply are considered 'lost.' Each different source of retreating or losing troops is handled separately" `[FAQ p. 1]` — `combat.finish_combat`이 정리에서 supply로 돌아가는 troop·Commander 수만큼 한 번 전진시키고, 한 출처가 여러 troop을 잃으면(Intrigue의 "troop N개 잃기" 비용, Tleilaxu Surgeon) slot마다가 아니라 한 번에 전진한다(끝을 넘는 초과분은 버림). 2026-09-26 정정: 이전에는 정리의 손실을 세지 않았고 다중 손실을 troop마다 전진시켰다.
...
- **Into the Fray(Signet)** — "You may take the Agent you sent this turn and deploy it to the Conflict as a 2 strength unit that can't be retreated. If you have your Swordmaster, it has 3 strength instead." `deploy_leader_agent`: 좌석 `agent_in_conflict`를 1 올리고(Servo-Receivers의 Signet으로 같은 round에 두 번째 Agent도 보낼 수 있다, OQ-037(e), 2026-09-26), 그 Agent는 `agent_locations`에서 빠져 공간이 다시 비고(OQ-037), `units_strength`가 2/3을 더하며, retreat·손실 대상이 아니다. Combat 정리 때 모두 `agents_available`로 돌아간다. 뒤의 turn에는 Imperial Privilege(`recall_conflict_agent_for_imperial_privilege`)뿐 아니라 모든 Recall Agent 효과 — Steersman의 Recall Agent 아이콘과 Sardaukar II·원로회(High Council) 계약 보상(둘 다 `recall_conflict_agent_for_agent_card` / `recall_conflict_agent_for_contract`) — 가 하나씩 recall할 수 있다(디자이너 판정 OQ-037(d), 2026-09-09; 모든 Recall Agent 효과로 확장한 2026-09-26 사용자 판정, OQ-068). Twisted Mentat는 예외로, 같은 turn에 Into the Fray가 옮긴 바로 그 Agent("the Agent you sent this turn")를 그 turn 안에 Conflict에서 recall할 수 있다(2026-09-26 사용자 판정, OQ-068).
...
- **Smuggle Spice(Signet)** — "Place 1 bonus spice on Tuek's Sietch. —OR— Take 1 bonus spice from a Maker board space." `place_leader_bonus_spice` / `take_leader_bonus_spice(space_id)`(bonus가 있는 Maker space만). "may"도 화살표도 없으므로 의무 택일이다 `[FAQ p. 3]`(2026-09-26 정정 — 이전에는 거절을 제시했다); 두 쪽 모두 불가능할 때만 안전장치로 거절이 남는다. Signet을 먼저 해결하고 같은 turn의 방문 행으로 가져가는 순서는 자유 순서 그룹이 그대로 허용한다 `[Bloodlines p. 12]`.

### Gaius Helen Mohiam

- **Clandestine** — "Each card you play has the Spy icon. Whenever you could recall a Spy to Gather Intelligence, you must." `effective_agent_icons`가 그녀가 play하는 카드에 Spy 아이콘을 더해 접근과 아이콘 세기(Slig Farmer, OQ-055)가 같이 따르고(2026-09-26; 이전에는 `card_can_access_space`의 접근 예외뿐이었다), `spies.legal_gather_intelligence_actions`는 회수 가능한 Spy가 있으면 거절을 빼고 제시한다. codec은 Bloodlines 카탈로그에서 모든 카드×모든 공간의 배치 템플릿을 갖는다.
...
- **Strange Form** — "You start the game with no water and without Signet Ring in your deck." `LeaderDefinition.starting_water=0`, `removed_starting_card_ids=("signet_ring",)`. 고정 setup과 Leader draft가 같은 `rules.setup.apply_leader_setup`을 쓴다(2026-09-26: draft에서 water와 Chani의 Tactics 시작 칸이 빠져 있었다).
- **Hungry for Spice** — "Whenever you gain 3 or more spice in a single turn: draw a card." 전이 후 hook `grant_hungry_for_spice`가 좌석의 `spice_gained_this_turn`(turn 시작 snapshot 기준, 다른 카드와 같은 정의)이 3 이상이면 turn당 1회 draw한다(`hungry_for_spice_granted_turn`, TURN frame이 열릴 때와 Round Start에 초기화). Y'rkoon 자신의 turn만 센다([OQ-063](../rules/open-questions.md#oq-063--hungry-for-spice의-in-a-single-turn)): 그 turn이 진행 중이거나 그 turn을 닫은 전이에서만 판정하고, Combat 보상·상대 turn의 획득은 세지 않는다. reshuffle chance가 열려 있는 동안은 판정만 하고 draw는 미뤄 두 reshuffle이 겹치지 않게 한다(`hungry_for_spice_owed`). 다른 좌석이 모두 Reveal해 닫는 전이가 곧바로 그의 다음 turn을 열면 `reset_turn_counters(..., closing=)`가 snapshot 전에 닫힌 turn을 판정해 같은 owed로 남긴다(2026-09-26 정정: 두 경우 모두 draw가 사라졌다).
