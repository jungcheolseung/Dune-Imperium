...
| RL encoding | Every transcribed physical card copy can take its Agent destinations, including Infiltrate variants. | Reserve actions grew the catalog from 554 actions in codec v6 to 754 in v7; the first Imperium batch grows it to 832 in v8. |
...
- The two printed card images linked by the Reserve content manifest were
  ...
  Reveal values.
- Maula Pistol and Truthtrance were bootstrapped from DIU `imperium.JSON` at
...
- All shared Imperium cards except Maula Pistol and Truthtrance still have only
