...
Spy that was recalled for the card. The recall stays optional ("you may first
recall one of your Spies for no effect" [Main pp. 11, 20], OQ-057 (14)): with
an empty supply the box counts as one that can only fizzle, so
`finish_agent_turn` is offered beside the recalls once nothing else is
pending, and ending the turn passes the recall up (2026-09-26).
