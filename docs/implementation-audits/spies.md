...
- `Gather Intelligence`: recall a connected Spy immediately after Agent
  placement and before board/card effects to draw one card.
...
Espionage adds one decline action and 13 each of recall and placement actions.
The fixed action catalog therefore changes from version 2 (428 entries) to
version 3 (455 entries), and new replay records default to codec version 3.
