...
| Agent-card placement | Bene Gesserit Operative selects any globally empty post; Reliable Informant is restricted to posts connected to Emperor, Bene Gesserit, or Fremen spaces (re-read from the card face 2026-09-26: the third target icon is the blue Fremen sietch badge, not the red Spacing Guild infinity symbol); Double Agent is restricted to posts connected to the space visited that turn, empty or holding another player's Spy, never the owner's own. | With an empty supply the recall first is optional [Main pp. 11, 20] (OQ-057 (14)): `decline_agent_card_spy` is offered beside the recalls until one is made, and passing resolves the box without a Spy while the Agent turn goes on, since the box and the space's effects resolve in any order [Main p. 9]; after a recall the placement is mandatory. The box needs that answer before the turn can end, like Espionage; with no Spy to recall it waits for the turn's end (OQ-057 (1)). A restricted effect offers only recalls that can open a destination when every eligible post is occupied. Arrakis Observer's Deep Cover Spy after its paid discard follows the same rule. Corrected 2026-09-26: the recall used to be forced and the turn could not end without it; the same day's first fix passed it up only through `finish_agent_turn`, so a Plot recall later in the turn (Special Mission) that refilled the supply made the placement mandatory again (codec v110 adds the decline). |
| Empty-supply pass-up, every path | Every Spy icon placed from the supply offers the pass-up of the optional recall-first [Main pp. 11, 20] (OQ-057 (14)): the shared `spy_placement` frame (Emperor track Influence 4, Panopticon, In High Places; `decline_spy_placement`), Espionage (`resolve_espionage_without_spy`), an Intrigue `PlaceSpy` slot (`decline_intrigue_spy`), Distraction (`decline_intrigue_trigger`), a Conflict reward (`decline_combat_reward_spy`), a Contract (`decline_contract_spy`), a Leader (`decline_leader_spy_placement`), a Reveal Spy icon (`decline_reveal_spy_recall`), an acquisition bonus (`decline_acquisition_spy`) and an Agent box (`decline_agent_card_spy`). | Each is offered only until a recall-first is made; afterwards the Spy is in the supply and must be placed. With a Spy in the supply none of them is offered while a post can take the Spy (Distraction's timing choice and printed "— OR —" alternatives aside). |
| Acquisition placement | Acquiring Strike Fleet or Guild Spy immediately opens a Spy placement choice over the current Reveal frame. | The acquired card is discarded and the row is refilled before choosing the post; with an empty supply the owner may recall first or pass (`decline_acquisition_spy`, OQ-057 (14)), after a recall the placement is mandatory, and either way the frame returns to Reveal purchasing. Corrected 2026-09-26: the recall used to be forced. |
...
`recall_spy_for_agent_card` again [Main pp. 11, 20]. The
`agent_card_spy_recalled` flag does not by itself mean a Spy is in the
supply, because the free ordering of Agent-turn groups lets the Espionage
board effect consume a Spy that was recalled for the card. The flag decides
only the pass-up: `decline_agent_card_spy` is offered beside the recalls until
a recall-first is made for the card, and afterwards the placement stays
mandatory (another recall, then the placement). The flag is cleared when the
box resolves, so a second Spy box of the same turn (a graft partner, Ghola's
copy) starts without it.
