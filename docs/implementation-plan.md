...
2. Bene Tleilax board 핵심: setup(token 2개, spice 2, Tleilaxu Row, Family Atomics, Experimentation 시작 덱), research 전진(두 방향 선택 frame, 보너스 해결, research 연쇄, genetic marker 판정, 두 번째 marker 뒤 draw 대체), Tleilaxu track(보너스, VP, 첫 도달 spice), specimen 생성·자유 반환·지출(troop 12개 불변식에 Axolotl tanks 포함), 개정 Research Station, Family Atomics 행동, 관측·codec 확장, 공식 문서가 침묵하는 판정의 OQ 등록.
