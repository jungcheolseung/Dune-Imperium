...
남은 콘텐츠(2026-09-03 사용자 결정): **Uprising 프로모 Imperium 3장** — Arrakis Revolt(Fremen, 비용 6, acquire troop, City 아이콘; Agent: Maker Hooks 조건으로 2 spice → Shield Wall 제거(선택) + sandworm, Reveal 1 Persuasion 3 sword), The Beast's Spoils(Emperor, 비용 3, City 아이콘; Agent: face-up battle icon 종류마다 보상 — Crysknife: trash, Desert Mouse: 1 spice, Ornithopter: troop; Reveal 3 sword), Pivotal Gambit(Fremen, 비용 3, Fremen·City 아이콘; Agent: 이 카드 trash → troop 그리고 이번 Conflict 1위 보상에 Influence 1 선택 추가, Reveal 1 Persuasion 2 sword). 정식 덱에 없는 후속 프로모지만 Uprising 양식이라 구현 대상이다(카드 텍스트는 위 요약이 아니라 에셋 저장소의 `uprising/promo/` 이미지로 검증한 뒤 전사한다; 덱 포함 여부는 ruleset option). **구현 완료(2026-09-03)**: 이미지 확대 검증 결과 세 효과 모두 **Agent box**(회색 상자)이고 파란 Reveal 상자는 Persuasion·검뿐이다. `RulesetConfig(promo_cards=True)`(identifier `+promo`, 서버·UI 체크박스·sweep `--promo-cards`)로 덱에 넣는다. 새 enum: `PersonalCardAcquisitionEffect.RECRUIT_ONE_TROOP`, `PersonalCardAgentEffect`의 `MAY_PAY_TWO_SPICE_FOR_SHIELD_WALL_AND_SANDWORM_IF_MAKER_HOOKS`·`GAIN_REWARDS_PER_FACE_UP_BATTLE_ICON`·`MAY_TRASH_SELF_FOR_TROOP_AND_FIRST_PLACE_INFLUENCE`; 상태 `conflict_first_place_influence_bonus`(관측 v4, codec v85). 공식 문서가 침묵하는 판정은 OQ-024~026에 project convention으로 기록했다. 이미지 캐시의 다른 프로모 6장은 각 확장팩의 프로모(base: Duncan Loyal Blade·Jessica of Arrakis·Thumper, Rise of Ix: Boundless Ambition, Immortality: Piter Genius Advisor, Bloodlines: Ruthless Leadership)라 범위 밖이다. 이 슬라이스는 M9보다 먼저 진행한다(사용자 결정).
...
진행(2026-09-07): 슬라이스 1~7 완료 — 슬라이스 6은 Ixian Embassy와 Tech tile 18장(획득·할인·Flip·획득 효과·능력), Tech 전용 Imperium 2장·Intrigue 2장, Kota Odax of Ix까지(관측 v9, codec v91→v94; 판정 OQ-040~045). 슬라이스 7(같은 날 밤): 웹 UI의 Commander·Skill·Ixian Embassy·보유 Tech tile·Secret Project 표시(`/catalog`의 `skills`·`tech` 절, tile 이미지 키), heuristic의 tile별 `acquire_tech` 가중치와 rollout `player_value`의 Bloodlines 자산, `--bloodlines --tech-module` 교차 소크 7,000판 실패 0(첫 소크가 적발한 3계열 결함 수정: Engineered Miracle의 만료된 Command 선택, Harkonnen Advisor troop만 있는 garrison의 배치 Plot 교착, Faction 없는 space의 Treacherous Maneuver — OQ-022·038 보강, OQ-046), 관측 v9·codec v94 기준 `dune-imperium-train` smoke(3 iteration, 체크포인트·평가 정상). 저장 문서가 `promo_cards`·`bloodlines`·`tech_module`을 잃던 결함도 함께 고쳤다. **M12 완료.** 후속 후보: 학습 루프에 `--bloodlines`/`--tech-module` 옵션(현재 `--choam`뿐), Endgame tiebreaker의 Commander 계수 판정. 2026-09-08: OQ-047(Endgame tiebreaker의 garrison Commander 포함), Bloodlines 프로모 Ruthless Leadership(`promo_cards`+`bloodlines` 옵션, 카드면 전사), 서버 AI 좌석에 `rollout`/`checkpoint:` 연결, UI의 Agent 토큰 애니메이션·1700px 패널 기준. 사용자 결정(2026-09-08): M10 학습은 최후순위로 두고 구현을 먼저 완결한다. 2026-09-09: 디자이너 판정 대조가 드러낸 콘텐츠 공백인 CHOAM Module contract token 8개(Earn Any Alliance·새 Immediate 포함)를 전사·구현했다(관측 v16, codec v98, OQ-056).
...
2026-09-08 착수(사용자 지시). 규범 근거는 [`rules/immortality.md`](rules/immortality.md)이며, 확장은 `RulesetConfig(immortality=True)` 옵션으로 켠다. Uprising Main p. 18의 "Adding Immortality"(Research Station overlay)가 조합을 공식 지원하고, Bloodlines와 독립이다. 기본값은 꺼짐이라 기존 룰셋의 codec 카탈로그는 바뀌지 않는다(관측은 identity 우주 확장으로 v11).

슬라이스 순서:

1. 출처 등록(`official-rule-sources.json`의 `immortality`, `sources.md`, `source-map.md`)과 규칙 명세, `RulesetConfig` 옵션 골격과 서버·CLI·저장·codec·coverage 연결, Bene Tleilax board 전사(`content/immortality/board.py`), Tleilaxu deck·Imperium 25·Intrigue 11 카탈로그, 카드면 57장 전사표(`implementation-audits/immortality.md`). **완료(2026-09-08).**
2. Bene Tleilax board 핵심: setup(token 2개, spice 2, Tleilaxu Row, Family Atomics, Experimentation 시작 덱), research 전진(두 방향 선택 frame, 보너스 해결, research 연쇄, genetic marker 판정, 두 번째 marker 뒤 draw 대체), Tleilaxu track(보너스, VP, 첫 도달 spice), specimen 생성·자유 반환·지출(troop 12개 불변식에 Axolotl tanks 포함), 개정 Research Station, Family Atomics 행동, 관측·codec 확장, 공식 문서가 침묵하는 판정의 OQ 등록. **완료(2026-09-08, OQ-048~051, 관측 v12, codec v96).**
...
6. 실전 점검(Tailscale)과 운영 문서. **리허설·운영 문서 완료(2026-09-17), 실제 친구와의 한 판은 남음**: [remote-play-guide.md](remote-play-guide.md), Tailscale 주소에 bind한 리허설 `scripts/e2e/rehearsal.py`(비보안 컨텍스트·쿠키·회선 에뮬레이션 실측), bind할 수 없는 `--host`를 링크를 찍기 전에 거절하는 CLI(`bind_problem`).
