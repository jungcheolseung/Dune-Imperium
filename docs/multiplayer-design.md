...
- 낙관적 동시성(`revision` + `undo_count` → 409), 되돌리기 창(`server/session_log.py`의 `undo_window`), 턴 종료 확인(`awaiting_confirmation`)이 이미 다인 전제로 설계돼 있다 — 다른 좌석이 행동하면 창이 닫히고, 되돌릴 수 있는 동안은 턴이 넘어가지 않는다. (2026-09-23 갱신: 이제 사람 좌석의 **모든** 턴 끝이 그 좌석의 "턴 종료" 한 번을 기다린다 — 되돌릴 것이 남았는지와 무관하고, 명시적 종료 행동(`finish_agent_turn`·`finish_reveal`·`pass_*_intrigue`)은 그 한 번 자체다. 판정은 `server/turn_end.py`와 세션의 `open_units`, 저장 문서에 `confirmation`·`sealed_steps`·`open_units`를 적는다.)
...
3. **갱신.** `applySummary`의 순차 GET을 snapshot 한 번으로 바꾸고, 갱신을 single-flight 루프(도는 중에 온 요청은 "한 번 더" 표시만)로 직렬화한다. 자기 POST가 진행 중일 때 온 초인종은 무시한다(POST 응답 뒤에 어차피 갱신한다). **불변식(슬라이스 4에서 확정): `state.summary`와 `state.me`는 `openGame`(첫 진입)과 `adoptSnapshot`만 쓴다.** snapshot은 한 줄로 서서 하나씩 요청되므로 채택되는 응답은 항상 앞의 것보다 나중에 읽힌 상태다. 그 줄 밖에서 보낸 요청은 늦게 도착해 화면을 과거로 되돌릴 수 있고, 그것이 마지막 초인종 뒤라면 아무것도 바로잡지 않는다(11절 슬라이스 4의 실측). 예외는 초인종의 공개 `players`를 `summary.players`에 그대로 옮기는 것 하나인데, 그때 snapshot이 비행 중이면(그 snapshot은 초인종보다 먼저 읽혔을 수 있다) 한 바퀴를 더 예약한다.
...
- **E2E**(슬라이스 4): headless Chromium의 브라우저 컨텍스트 둘(쿠키 분리)로 방 생성 → 입장 → claim → 몇 턴 → 새로고침 복귀 → release → 재claim, JS·서버 오류 0. Playwright는 프로젝트 의존성에 넣지 않고 스크래치 venv로 돌리되, **스크립트는 저장소의 [`scripts/e2e/`](../scripts/e2e/README.md)에 둔다**(처음에는 M11 때처럼 세션 스크래치에만 뒀다가 다음 세션·다른 기기에서 전부 다시 만들어야 했다): `remote.py`(이 시나리오 + 매 스텝 뒤 "모든 페이지가 서버 상태로 수렴" 검사), `open_mode.py`(슬라이스 2·3의 open 모드 E2E 3종), `races.py`(응답 순서를 `page.route`로 강제하는 개입 실험).
...
4. **클라이언트 원격 UX.** 좌석 고르기·이름, 내 좌석 고정, 대기 배너·접속 표시·차례 알림, 팝오버·스크롤 보존, 호스트 패널, 설정 화면의 관리자 제한, `textContent` 점검, E2E. 완료 조건: 10절의 E2E 시나리오 통과. **여기까지가 첫 원격 한 판의 최소 구성이다.** **완료(2026-09-17).** 구현 메모: (a) 서버 쪽 — `GET /whoami`(`access`·`admin`·관리자에게만 `public_url`), CLI `--public-url`, 그리고 remote 서버에서만 턴 종료 확정 대기 중에는 다음 좌석이 행동할 수 없고 snapshot의 `actions`도 `null`이다(`_turn_is_held_locked`; open 서버의 기존 API 계약은 그대로). (b) 화면은 넷이다 — 설정(관리자), landing(링크 없이 온 손님; 마지막으로 있던 방으로 "이어 하기"), 좌석 고르기, 테이블. 방 링크는 `#game=<id>`이고 `#admin=<key>`는 쿠키로 바꾼 즉시 주소창에서 지운다. hash와 localStorage에서 온 game id는 형식을 검사한 뒤에만 요청 경로에 쓴다. (c) **WIP 클라이언트의 멈춤은 계측으로 원인을 확정했다**: players만 바뀐 초인종에서 `seatsChanged()`가 single-flight 밖에서 seatless snapshot을 받아 `state.summary`를 덮어썼는데, 그 응답 하나를 0.7초 붙잡고(읽기는 즉시) 그 사이 상대가 턴 종료를 확정하게 하면 기다리던 페이지의 이력이 `13/0/1 → 13/null/1 → 13/0/1`(revision/confirmation/owner)로 되돌아가, 서버는 "네 차례"인데 화면은 "좌석 0의 턴 종료 확정을 기다리는 중"인 채 영구 정지했다(초인종이 더 올 일이 없다 — 양쪽이 서로를 기다린다). 고침: 7절 3항의 불변식 — `reloadIdentity()`·`seatsChanged()`를 없애고 claim·release·좌석 변경을 전부 `refresh()`로(먼저 물을 좌석을 `{seat}`로 넘길 수 있다), 좌석 상실은 `adoptSnapshot`이 "좌석이 있었는데 이제 없다"로 감지한다. (d) 같은 부류의 둘째 경합을 수렴 검사가 8회에 1회꼴로 잡았고 개입으로 재현했다: 입장하는 페이지는 스트림을 열면서 곧바로 snapshot을 요청하는데, 그 snapshot이 자기 스트림 연결보다 먼저 읽히고(`online: false`) 연결 초인종보다 늦게 도착하면 옛 `players`가 되살아난다(스트림 0.3초·snapshot 0.9초 지연으로 재현: 예약 없이는 stale, 있으면 수렴). 그래서 players 초인종이 비행 중에 오면 한 바퀴를 더 돈다. (e) players만 바뀐 초인종은 **내 좌석의 `claimed`·`name`이 바뀐 때만** 요청을 만든다(호스트의 release는 게임 step 없이 좌석을 가져가고 payload는 누구의 claim인지 말하지 못한다). 접속 점만 바뀐 알림은 payload를 그대로 쓴다 — 요청 0개. 내 좌석이 비었다고 나오면 그 좌석으로 묻지 않고 seatless로 물어 403을 만들지 않는다. POST의 403은 refresh를 거쳐 좌석 고르기로 간다. (f) claim·release도 다른 POST처럼 `state.busy`다(자기 초인종에 요청으로 답하지 않고 이중 클릭을 막는다). claim 응답이 나간 뒤에 도착해도 테이블을 다시 띄우지 않는다(`openTicket`). (g) `[hidden] { display: none !important }` — `fieldset label { display: block }`이 UA의 `[hidden]`을 이겨 remote 호스트에게 Seed 칸이 보이던 결함(E2E가 적발). (h) 서버: `@app.middleware("http")`(BaseHTTPMiddleware)는 응답 시작 전에 끊긴 요청마다 `RuntimeError("No response returned.")`를 내는데, 스트림을 다시 여는 페이지는 방금 요청한 `EventSource`를 닫는다 — 1 ms 안에 닫은 소켓 4개 중 4개가 uvicorn ERROR 트레이스백을 남겼다. `no-cache` 헤더를 순수 ASGI wrapper(`_RevalidateUIFiles`)로 옮겨 0/7. (i) 검증: `scripts/e2e/remote.py` 56항목 5회 연속, `open_mode.py` 44항목(300 스텝 = POST 300 = snapshot 300, 로그 일치, 검토 경합, 초인종 30 ms·폴링 1.5초·삭제 통지), `races.py --ab`, 실패한 요청·JS 예외·서버 오류 0. 리뷰 워크플로(5개 관점 → 반박 검증)의 확정 2건(설계 7절 1항의 "이어 하기" 누락, claim 도중 나가기)을 반영했다. (j) 남긴 것: 7절 8항의 호스트 패널 중 "저장·자동 저장 목록"은 슬라이스 5와 함께, 실제 WAN 한 판은 슬라이스 6.
5. **자동 저장과 복구.** `write_autosave`, 턴이 넘어갈 때의 저장, 복구 절차. 완료 조건: 서버 강제 종료 → 재시작 → 자동 저장에서 이어 가기. **완료(2026-09-17).** 구현 메모: (a) **언제**는 세션 계층이 정한다 — `GameSessionManager.add_hand_over_listener(cb)`의 `cb(game_id)`는 `confirm_turn` 뒤에는 항상, `apply_action` 뒤에는 "턴이 넘어갔을 때"만 불린다(2026-09-24부터 명시적 종료 행동 뒤에도 항상 — 그 행동이 곧 턴 종료의 누름이다)(`_turn_passed`: 확정 대기 중이 아니고, 게임이 끝났거나 · 자기 step 뒤에 다른 좌석의 action이 이어졌거나 · 결정이 다른 좌석에 가 있을 때; chance 결과만 이어지고 같은 좌석이 턴을 계속하면 아니다). 되돌리기·claim·release·접속 변화는 부르지 않는다. 초인종과 같은 자리(변경을 만든 스레드, **lock을 푼 뒤, 초인종 publish 뒤**)에서 불리고, listener의 예외는 로그만 남기고 삼킨다 — 그 step은 이미 적용됐고 플레이어는 응답을 받아야 한다. (b) **무엇을**은 `server/autosave.py`의 `Autosaver`: 한 lock 안에서 문서를 만들고(`save_document`, 자격 검사 없음·route 없음) 쓴다. 두 요청이 거의 동시에 끝나도 나중에 쓰는 쪽이 나중에 읽은 쪽이라 옛 상태가 새 상태를 덮지 못한다. 이름은 "자동 저장 · R<n>"/"자동 저장 · 종료". 실측(2026-09-17, Mac mini, 사람 3 + heuristic 1을 끝까지, seed 20260917): base 한 판에 자동 저장 112회·호출당 중앙값 3.1 ms·최대 6.1 ms·합계 0.36초(마지막 파일 179 KB), 전 확장은 89회·3.2 ms·최대 8.2 ms·합계 0.30초(170 KB). (c) `SaveStore.write_autosave`: 슬롯 = game id(저장 ID와 같은 모양), `"autosave": true`, `os.replace`로 원자적 교체. 목록·불러오기·삭제는 보통 저장과 같은 경로다. 불러온 게임은 새 id라 **자기 슬롯**에 저장한다. 게임을 삭제해도 자동 저장 파일은 남는다(호스트가 저장 목록에서 지운다). (d) 기본값은 접근 모드를 따른다(remote 켬·open 끔); `--no-autosave`(‑‑remote 없이 주면 거부), `create_app(autosave=)`, `/whoami`의 `autosave`. 시작할 때 콘솔에 저장 폴더와 복구 절차를 찍는다. (e) 클라이언트: 설정 화면 저장 목록의 "자동 저장" 배지와 현지 시각, 호스트 패널의 저장 블록(이 게임의 저장 목록 — `GET /saves`는 모든 파일을 읽으므로 패널을 열 때·저장 직후·버튼으로만 부른다 — 과 복구 절차 안내, "지금 저장"), 폴링이 아무 응답도 못 받으면 "서버 연결 끊김" 표시, 폴링의 404는 "삭제 또는 재시작"이라 "호스트의 새 방 링크로 들어오세요"로 안내하고 죽은 방을 "이어 하기"로 제안하지 않는다(스트림의 `closed`는 여전히 "삭제됨"). (f) 검증: `tests/server/test_autosave.py`·`test_autosave_app.py`·`test_server_cli.py`(실제 프로세스를 SIGKILL → 같은 저장 폴더로 재시작 → 불러오기, revision = 자동 저장의 step 수), 브라우저 [`scripts/e2e/recovery.py`](../scripts/e2e/README.md) 32항목(파일 하나·`.tmp` 잔재 없음·한 턴 이내, 친구 화면의 끊김 표시와 안내, 호스트의 불러오기, 둘 다 새 링크로 복귀해 16 스텝 수렴). (g) 리뷰가 드러낸 **기존 결함 하나를 따로 고쳤다**: `confirm_turn`은 되돌리기 창을 닫는다고 약속하지만 창은 로그에서 읽히고 로그는 다음 chance·다른 좌석 step에서야 닫힌다 — 다음 좌석이 아직 두지 않은 **사람**이면 확정한 좌석이 되돌리기로 턴을 다시 가져올 수 있었다(한 화면에서는 보이지 않고 두 브라우저에서 보인다). 세션의 `undo_floor`(마지막 확정 시점의 live step 수)가 창을 자른다. 저장하지 않으므로 바로 그 순간에 저장된 게임은 불러온 뒤 그 좌석에게 확정을 한 번 더 묻는다(기존 동작; 2026-09-23부터는 저장 문서가 `sealed_steps`·`confirmation`을 담아 해소 — 그 필드가 없는 옛 문서만 이렇게 된다). (h) 남긴 것: 자동 저장은 동기(요청 스레드)다 — 저장 폴더가 멈추는 파일시스템이면 턴을 넘기는 요청이 같이 멈춘다(수동 저장도 같다; D6이 비용을 재고 고른 구조).
6. **실전 점검과 운영 문서.** README의 호스트 절차(Tailscale, `--remote` 명령, WSL 주의), 실제 친구와 한 판, 피드백 반영, 필요하면 보드 스캔 축소본. **리허설과 운영 문서 완료(2026-09-17) — 다른 집의 친구와 인터넷 너머로 하는 한 판은 남았다.** 구현 메모: (a) 운영 문서 [remote-play-guide.md](remote-play-guide.md)(호스트의 첫 설정·판을 열 때마다·친구에게 보낼 안내·복구·첫 실전 판의 점검표). Tailscale 공식 문서를 다시 확인했다(2026-09-17): 초대 링크(재사용 가능·30일 만료), 공유된 머신은 격리 상태, 그리고 **받는 쪽 tailnet에서 호스트의 100.x 주소가 다를 수 있다** — 그때는 친구가 방 링크의 주소 부분을 자기 쪽 주소로 바꾼다. (b) 리허설 [`scripts/e2e/rehearsal.py`](../scripts/e2e/README.md): 이 Mac mini의 Tailscale 주소에만 bind한 서버에 그 주소로 들어가(같은 머신이므로 WAN은 아니다) `isSecureContext === false`, 복사 버튼의 선택 fallback, 쿠키 속성(HttpOnly·SameSite=Strict·게임 경로 한정·`Secure` 아님)을 확인했고, 같은 주소에서 `remote.py` 56·`recovery.py` 32·`races.py --ab`가 통과했다. 그 주소에만 bind하면 `127.0.0.1`로는 연결이 거부된다. (c) 회선 에뮬레이션 실측(전 확장, 캐시 없는 첫 접속, 두 번 같은 값): 100 Mbit/s·RTT 20 ms에서 둘 수 있는 테이블 0.2초·모든 이미지 1.8초·18.5 MB·한 수의 반영 29 ms; 10 Mbit/s·RTT 120 ms에서 0.8초·16.3초·18.6 MB·158 ms; 새로고침은 둘 다 0.03 MB(보드 스캔이 캐시된다). 느린 회선에서도 테이블은 1초 안에 둘 수 있고 보드 그림만 늦으므로 **축소본은 만들지 않았다.** (d) 리허설이 드러낸 CLI 결함을 고쳤다: bind할 수 없는 주소(Tailscale이 꺼져 있을 때의 100.x)를 주면 uvicorn의 실패 로그 사이에 **뜨지도 않은 서버의 관리자 링크**가 찍혔다 → `bind_problem`이 먼저 검사해 링크 없이 이유를 말하고 종료 코드 1로 끝난다. 시작 출력은 flush해서 파이프·파일로 받아도 uvicorn 로그보다 앞에 온다. (e) 확인하지 못한 것: 실제 WAN 너머의 판, WSL2 호스트, 신호음의 실제 재생(자동 재생 정책), 몇 시간짜리 연결 유지.

후속(필요해질 때): AI worker와 단계별 푸시(4.8절의 불변식), AI 대타(좌석이 사람→AI로 바뀐 시점을 기록하는 저장 형식 v3가 필요 — 지금의 불러오기는 AI 좌석의 모든 step을 agent로 재생성해 대조하므로 도중에 주인이 바뀐 좌석을 재생할 수 없다), 관전자(공개 전용 view가 `core/observation.py`에 필요하므로 R5 밖; 사람 좌석이 없는 게임만은 2026-09-20부터 끝난 게임의 리플레이 검토를 자동 재생해 본다 — 그런 게임은 생성 요청 안에서 끝까지 돌고 끝난 게임은 전부 공개라 view도 권한도 새로 필요 없다), 공개 터널용 에셋 게이트, 이름의 저장 파일 보존, 행동 POST가 snapshot을 바로 반환.
