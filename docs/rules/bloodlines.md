...
- 2026-09-07 슬라이스 4d-2: Urgent Shigawire(round 한정 좌석 플래그, 관측 v7), Sardaukar Standard(trash 트리거 → 대기열 → Skill 선택 frame, OQ-035), Litany Against Fear(turn 시작 대체 행동), Delivery Logistics(contract 아이콘 차용, Persuasion/contract 선택), CHOAM Demands(contract 임의 완료, 4 진영 Influence trash 선택).
- 2026-09-07 슬라이스 4d-3: Holy War(상대 유닛 손실 + Spy 강제 이동, OQ-036; Fremen Bond Combat 아이콘), False Orders(Spy 강제 이동 뒤 자기 Spy 배치), Coercive Negotiation(배치 3+ trigger → bank contract 3장 공개·1장 획득·2장 `contract_trash`). 이로써 Tech 전용 카드를 뺀 Bloodlines Imperium 25종·Intrigue 16장이 모두 play된다.
