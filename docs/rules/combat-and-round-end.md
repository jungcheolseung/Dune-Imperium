...
- Combat 보상으로 받은 Intrigue 가운데 "Conflict 끝에 troop을 잃으면" trigger 카드(Immortality의 Harvest Cells)는 보상 해결 뒤·정리 전에 열리는 창에서 즉시 play할 수 있다(디자이너 판정 채택, [OQ-057](open-questions.md#oq-057--디자이너-커뮤니티-판정의-일괄-채택-2026-09-09)). 창은 First Player부터 turn 순서다. play한 카드는 face-up으로 대기하다가 정리에서 troop이 supply로 돌아간 뒤(그 반환이 "lost"다 `[FAQ p. 1]`) 발동하므로, specimen은 돌아온 troop으로 만든다 `[Immortality p. 8]`. [Main p. 14]
