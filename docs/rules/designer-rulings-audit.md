...
상태: 대조 완료, **13건 전부 반영 완료**(2026-09-09; 아래 "반영 현황" 절). 이 문서는 규범 규칙이 아니라 **감사 결과**다. 항목을 엔진에 반영할 때는 [open-questions.md](open-questions.md)에 OQ-022와 같은 방식(`DECIDED`, 출처는 공식 문서가 아닌 디자이너 포럼 판정)으로 등록하고, 규칙 문서·테스트를 같은 변경 단위로 고친다.
...
| Call to Arms: Intrigue draw는 세지 않고, 소급하지 않으며, Agent turn에 play해 face-up 대기 | `rules/intrigue_triggers.py` `fire_reveal_acquisition_intrigue`(personal card 획득 경로에서만 호출 — 2026-09-26부터 Tleilaxu Row 획득 `acquire_tleilaxu_card`도 포함 `[Immortality p. 8]`; Reclaimed Forces의 "acquire"도 2026-09-26부터 발동한다 — 사용자 판정, OQ-066) |
...
사용자 결정: 13건 전부 디자이너 판정을 따른다. 13건 모두 반영했다(표의 각 행 앞 표시; 판정 등록은 [OQ-057](open-questions.md#oq-057--디자이너-커뮤니티-판정의-일괄-채택-2026-09-09), OQ-002·OQ-028·OQ-037·OQ-054 보강). 7번의 순차 지불도 같은 날 OQ-058(사용자 판정: Intrigue 줄은 각각 따로 쓰고 쓸 때 지불)로 해소했다.
