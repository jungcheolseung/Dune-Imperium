...
- Reveal box(미적용, 다음 작업): Reveal의 자동 이득(Persuasion·sword·자원·troop·Intrigue draw·Influence)은 Reveal 시작 시 한꺼번에 적용되고, 선택형 Reveal 효과는 카드 순서대로 쌓인 `REVEAL_CHOICE` frame으로 직렬 해결된다. `[Main p. 12]`("You may resolve Reveal effects in any order you like... use Persuasion at any time")에 맞추려면 선택형 효과를 소유자가 고른 순서로, 그리고 획득과 섞어 해결할 수 있어야 한다. 순수 이득의 시점은 결과에 영향을 주지 않으므로 선택형 효과의 순서 자유화(예: 선택 frame `defer`/`resume`)만 남았다.
