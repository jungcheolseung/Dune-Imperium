...
- 판정(사용자, 2026-09-07): troop recruit와 Intrigue draw는 카드 draw와 같은 **각각 하나의 효과**로 보고 소유자가 Reveal 안에서 시점을 고른다. 구현: Reveal 시작 시 카드·tile의 troop 아이콘과 Intrigue draw 아이콘을 Reveal frame의 대기 목록(`reveal_pending_gains`)에 넣고, `recruit_reveal_troops`/`draw_reveal_intrigue` 행동이 가장 오래된 같은 종류 항목 하나를 해결한다(같은 종류끼리는 supply·덱이 같아 순서가 무의미하므로 인자 없음). 늦게 조건이 성립한 효과(OQ-028)와 Reveal 중 도착한 카드(FAQ p. 3)의 troop·Intrigue도 같은 목록에 들어간다. 이득이므로 거절은 없고, 남아 있으면 `finish_reveal`이 제시되지 않는다. Persuasion·sword·자원·Influence는 시점이 결과를 바꾸지 않으므로 그대로 시작 시 적용한다(자원 중 spice는 Forbidden Weapons의 "spice 전부 잃기" 앞뒤로 차이가 날 수 있으나, 룰 문면상 tile 효과를 먼저 고르는 것으로 대비 가능하며 별도 판정 대상으로 남긴다). troop recruit의 `troops_recruit_short`(OQ-030)와 Combat 아이콘 배치 창의 recruit 합산은 행동 해결 시점에 일어난다. codec v92(모든 카탈로그 +2).
