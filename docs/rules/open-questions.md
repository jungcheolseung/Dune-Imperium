...
- 판정 후보(권고, 미확정): (A) 해결 시점 판정·소급 없음 — 부족분은 소멸하고 플레이어가 자유 순서로 supply를 먼저 채운다. OQ-028의 해결 시점 원칙 및 `[Immortality Rules p. 8]`의 "미리 되돌려 두라"는 안내와 같은 방향이며 엔진 변경이 없다. (B) 같은 turn 안에서 supply가 늘어나면 부족분만큼 늦게 recruit — OQ-028(c)의 `granted_reveal_effects`처럼 turn frame에 부족분을 기록하고 supply 증가 뒤 자동 지급해야 하며, Combat space의 배치 한도(`troops_recruited`)도 함께 늘려야 한다.
- 재개 조건: 사용자 판정, 또는 새 공식 룰북·FAQ, 또는 turn 중에 troop을 supply로 돌려보내는 콘텐츠(Immortality의 lose a troop·specimen 등)를 구현 범위에 넣을 때.
