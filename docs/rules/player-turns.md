...
- troop icon 하나를 처리할 때 자신의 supply에서 troop 하나를 가져와 board의 garrison에 놓는다. supply에 troop이 없으면 recruit할 수 없다. [Main p. 10]
  프로젝트 판정(OQ-030 `DECIDED`, 2026-09-06, 공식 규칙이 아닌 convention):
  recruit는 해결 시점에 supply에 있는 만큼만 일어나고 부족분은 소멸하며, 같은
  turn 안에서 supply가 뒤늦게 늘어나도 소급하지 않는다. 플레이어는 자유
  순서로 supply를 채우는 효과를 먼저 해결해 대비한다. 부족분은 공개 이벤트
  `troops_recruit_short`로 로그에 남긴다. 근거와 확장 사례는
  [open-questions.md](open-questions.md)의 OQ-030.
...
4. Reveal 효과는 원하는 순서로 처리한다. Persuasion을 사용한 acquire는 Reveal 효과의 전, 사이, 뒤 어느 때든 할 수 있다. [Main p. 12] 선택형 Reveal 효과는 미루기/재개로 순서를 고르고(OQ-027), troop recruit와 Intrigue draw는 각각 소유자의 행동(`recruit_reveal_troops`·`draw_reveal_intrigue`)으로 시점을 고르며(OQ-045), 조건이 있는 Reveal 효과는 같은 Reveal 안의 뒤 선택(Spy 배치, High Council 구매, 늦게 도착한 카드, 획득으로 완료된 contract)으로 조건이 성립하면 그때 지급된다(OQ-028).
