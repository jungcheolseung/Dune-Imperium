...
**남은 것** — 이번 범위 밖에서 본 것:
1. **패널·보드 문구의 용어집 잔재 11곳.** 용어집에 한국어가 있는데 영어로 남은 크롬: `Hand (공개)`(핸드),
   `Intrigue 덱 맨 위`(책략 카드덱), `Intrigue 해결 중`, `내 discard`(버림 더미), `Navigation N장 남음`(운항
   카드), `Combat Intrigue 창`(전투 책략 카드), `(Flip됨)`(뒤집기), `Tactics N칸`(전술 트랙), `보드에 남은
   Commander 없음`(사다우카 지휘관), `Ixian Embassy`(익스 대사관 판), 설정 화면의 확장 이름(혈통·불멸·봉기,
   용어집 "적용"). `lang.py`의 이름 없는 크롬 검사가 이 자리들을 보지 않는다.
