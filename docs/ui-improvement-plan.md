...
- **5d.** 머리글의 **English / 한국어** 버튼이 화면 언어를 바꾸고 브라우저에 기억된다(기본 한국어). 한 판의
  ...
  **남은 것(다음 단위).** 검증이 찾은 **한국어 모드의 영어 잔재 27건** — 공용 카드 열 제목(Imperium Row·
  Reserve·Tleilaxu Row·Bene Tleilax board; 접힘 상태의 저장 키이기도 해서 키를 언어와 분리해야 한다),
  "Maker Hooks"·"bank"·"stack", 순위표 머리글(VP·Spice…), `YOU`/`1st`/`AI` 배지, CHOAM Module 체크박스,
  `{tech_tile} Flip` 등. 영어 모드에는 영향이 없다.
