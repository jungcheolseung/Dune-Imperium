...
| `seats.py` | 좌석 패널 접기(2026-09-20). 좌석 넷이 카드 줄을 "자세히"로 접는지, 접혀도 무엇이 들었는지 말하는지, 한 좌석만 펴도 나머지는 그대로인지, 재렌더·새로고침을 견디는지, `s`가 전부 펴고 접는지, 접은 상태가 실제로 칸 높이를 줄이는지(후반 2,189 → 936px). 존 수치 줄이 영어로 되돌아가지 않았는지도 본다. 보드에서 바로 보이는 영향력·주둔지·교전 병력·전투력·연구/틀레이락스 위치와 병력 공급처는 상태창에 중복하지 않고, 표본은 아이콘과 수량으로 보이는지도 확인한다. 이어 **노트북 1366x768**(2026-09-21): 같은 판의 25/50/75/95% 네 시점 모두 접힌 좌석 넷이 칸에 들어오는지(칸이 519~652px로 바뀐다 — 손패 줄이 두 줄이 되면 줄어든다; 옛 패널은 703~738px), 접기가 머리 줄 끝의 화살표이고 누를 만한 폭인지, 그림이 빠진 자리에서도 지도자 이름이 보이는지, 화살표가 펴고 접는지. 1600x1000에서는 지도자 그림 넷이 그대로 있는지도 본다(높이 960px 이하에서만 압축). |
