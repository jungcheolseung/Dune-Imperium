...
(d) Steersman Y'rkoon's four Navigation-card slots [Bloodlines p. 12]: a row
    ...
    reviewer's own bug report exactly (2026-09-25): in an all-heuristic
    Bloodlines + Leader Draft game (found by `_find_navigation_review_case`),
    one step after a `play_navigation` the card that step actually played has
...
def leader_popover_korean_text(page, lang: str) -> None:
    ...
    card's engine-derived line); her Signet reward (two draw icons, not
    troops, docs/implementation-audits/leaders-ko.md finding 1) once
    disagreed with the English/engine, both since corrected to match this
    Korean transcription. Kota's own ability
...
def _find_navigation_review_case(
    base: str, *, limit: int = 40
) -> tuple[int, int, int, str, str] | None:
    """(seed, seat, step, played card, next front card) of an all-AI
    Bloodlines + Leader Draft game where a seat's ``play_navigation`` step
    resolves one Navigation card while the review view one step later
    already shows a DIFFERENT card at the front of that seat's
    ``navigation_slots`` -- the exact shape the review-label check below
    needs. Found over raw HTTP (``/games/{id}/review`` and
    ``/review/{step}``) instead of hard-coding a step: a rules change
    upstream shifts every later log index (seed 2's case moved from step
    120 to 145 when master's 2026-09-26/27 rulings landed)."""
    ...
    for seed in range(limit):
        ...
        for seat in range(4):
            log = _api_get(base, f"/games/{game_id}/review?seat={seat}").get("log") or []
            for step, entry in enumerate(log):
                if entry.get("action_id") != "play_navigation" or entry.get("actor") != seat:
                    continue
                played = [
                    event["payload"]["card_id"]
                    for event in entry.get("events") or []
                    if event.get("kind") == "navigation_card_played"
                ]
                if not played:
                    continue
                after = _api_get(base, f"/games/{game_id}/review/{step + 1}?seat={seat}")
                slots = ((after.get("view") or {}).get("private") or {}).get(
                    "navigation_slots"
                ) or []
                if slots and slots[0] != played[0]:
                    return seed, seat, step, played[0], slots[0]
    return None
...
def review_label_repro(base, browser) -> None:
    ...
    `_find_navigation_review_case` finds an all-heuristic Bloodlines +
    Leader Draft game where a seat's `play_navigation` step resolves one
    card while the review view one step later already shows the NEXT card
    at the front of `view.private.navigation_slots` (at the time of
    writing: seed 2, Steersman Y'rkoon on seat 3, `navigation_card_9`
    played at step 145 with `navigation_card_1` in front afterwards).

    `enterReview(seat, {cursor: step + 1})` is called directly (a page-context
    ...
    case = _find_navigation_review_case(base)
    check.ok(case is not None, "an all-AI game with the Navigation review case exists")
    if case is None:
        return
    seed, seat, step, played_id, front_id = case
    cursor = step + 1
    context, page, rec = open_context(browser, "review-repro", LAPTOP_VIEWPORT)
    _open_all_ai_bloodlines_game(page, base, seed=seed)
    ...
    # runs) -- the seat's leader_id is only meaningful once enterReview has
    # actually moved to that seat's own cursor.
    page.evaluate(
        "async ([seat, cursor]) => { await enterReview(seat, { cursor }); }",
        [seat, cursor],
    )
    page.wait_for_function(
        "(cursor) => state.review && state.review.cursor === cursor", arg=cursor
    )
    check.ok(
        page.evaluate(f"state.view.players[{seat}].leader_id") == "steersman_y_rkoon",
        f"seat {seat} really drafted Steersman Y'rkoon at seed {seed}",
        page.evaluate(f"state.view.players[{seat}].leader_id"),
    )
    catalog = page.evaluate("state.catalog")
    played_name = catalog["intrigue"][played_id.split(":")[1]]["name"]
    front_name = catalog["intrigue"][front_id.split(":")[1]]["name"]
    ...
    log_head = page.evaluate(LOG_HEAD_FOR_INDEX_JS, step)
    check.ok(
