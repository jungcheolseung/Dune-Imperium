...
| `train_overnight.py` | `run_guard.py` 아래에서 `dune-imperium-train`을 TOTAL iteration이 `training.jsonl`에 쌓일 때까지 돌린다. 예외로 죽으면 traceback 꼬리를 `crash_<n>.log`에 남기고 `--seed`를 1 올려 `latest.pt`에서 재개한다(같은 seed는 죽은 게임을 그대로 다시 밟는다). 메모리 중단, 수동 중지(`kill -TERM $(cat RUN/supervisor.pid)`), 두 번 연속 무진전, 8회 재시작 뒤에는 멈춘다. 끝나면 `supervisor.log`에 `supervisor-exit: ...`를 적는다. |
