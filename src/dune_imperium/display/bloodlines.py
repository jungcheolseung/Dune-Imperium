...
def skill_effect_text_ko(skill: SkillDefinition) -> str:
    ...
    if skill.reveal_troops:
        parts.append(f"{{reveal_turn}}: {{troop:{skill.reveal_troops}}}")
    if skill.trash_for_strength:
        parts.append(
...
def tech_acquire_text_ko(tile: TechTile) -> str:
    ...
    ``acquire_leader_signet`` (Servo-Receivers) reuses ``tokens_ko.py``'s own
    established ``LEADER_SIGNET`` phrasing "당신 {leader}의 {signet_ring}
    능력", with "사용" (use) added for English's own "Use ... ability" verb;
    Servo-Receivers' own scan confirms {signet_ring} for "Signet Ring"
    ("당신의 인장 반지는 다음 아이콘을 보유:" [KO card: Servo Receivers]).
    """
    ...
    if tile.acquire_leader_signet:
        parts.append("당신 {leader}의 {signet_ring} 능력 사용")
    if tile.acquire_solari:
