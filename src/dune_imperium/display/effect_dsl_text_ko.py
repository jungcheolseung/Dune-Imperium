...
def _place_spy_text_ko(spy: PlaceSpy) -> str:
    ...
    ``agent_icons`` (Special Mission's own Agent-icon-limited post, "'[Spy]
    on [City]' means the observation post must connect to a [City] board
    space" [Main p. 20]) mirrors the ``factions`` branch, naming the icon
    with ``_AGENT_ICON_NAMES_KO`` (this module's own established plain-word
    table, above).
    """
    ...
    if spy.agent_icons is not None:
        names = " 또는 ".join(
            _AGENT_ICON_NAMES_KO[icon] for icon in spy.agent_icons
        )
        return f"{{spy}} 배치 ({names} 관측소)"
    return "{spy} 배치"
...
def _gain_influence_text_ko(gain: GainInfluence) -> str:
    ...
    Mirrors it field-for-field, including ``where_opponent_leads``
    (Twisted Ambitious: "a Faction where an opponent has more Influence
    than you" [Twisted Ambitious card face]) and ``different_from_trigger``/
    ``minimum_own`` (Navigation card 1: "a different Faction ... where you
    have 2+ Influence" [Navigation Card 1 face]) — master's own English
    fix started rendering these two fields (they used to fall through
    silently), so this twin follows suit rather than leaving them out.
    "2+ Influence" draws **no** icon at all in English: unlike Persuasion
    (which has its own bare ``\bPersuasion\b`` ``ICON_RULES`` fallback, the
    reason "Command (6+ Persuasion)" still shows a bare Persuasion icon,
    ``display/tokens_ko.py``'s own docstring example), Influence has no
    such bare fallback rule -- only the counted/Faction-specific patterns,
    all of which require the digit immediately followed by " Influence"
    with no "+" in between -- so "2+" breaks every one of them and the
    word "Influence" itself stays plain text (2026-09-25 full-catalog
    sweep); this twin uses the plain word 영향력 here, never a placeholder,
    to match that. Non-distinct/
    "any Faction" wording quotes ``display/structs.py``'s own
...
def condition_text_ko(condition: Condition) -> str:
    ...
    match condition:
        ...
        case SandwormsInConflictAtLeast(count=1):
            # "you have one or more sandworms in the Conflict" [Devour card
            # face] [Ripples in the Sand card face] — the owner's own
            # sandworms, per effect_interpreter.py's owner-only check; "당신의"
            # names the owner the way the English fix now does.
            return "{conflict}에 당신의 {sandworm}가 있다면"
        case SandwormsInConflictAtLeast(count=count):
            return f"{{conflict}}에 당신의 {{sandworm}}가 {count} 이상 있다면"
...
def cost_text_ko(cost: Cost) -> str:
    ...
    match cost:
        ...
        case LoseTroops(count=count, from_conflict=from_conflict):
            # "Lose two of your troops" [Twisted Sinister card face] --
            # master's fix now prints the count ("Lose 2 troops"), which
            # sits directly next to "troops" and so draws the *counted*
            # troop icon, not the bare one this used to match (2026-09-25
            # full-catalog sweep). "Lose (troops) | 잃다" [Bloodlines p. 12].
            where = "{conflict}에서 " if from_conflict else ""
            return f"{where}{{troop:{count}}} 잃음"
        ...
        case FlipBattleCard(icon=icon):
            ...
            # takes a Wild-icon card. "Flip (a Tech tile) | 뒤집기"
            # [Bloodlines p. 12] — the same terse nominal verb reused for a
            # Conflict card; "앞면" (face-up) matches
            # ``FlipFaceUpConflictCard``'s own established wording below.
            icon_name = _BATTLE_ICON_NAMES_KO[icon]
            wild_name = _BATTLE_ICON_NAMES_KO[BattleIcon.WILD]
            return (
                f"이긴 앞면 {{conflict}} 카드 하나 뒤집기 "
                f"({icon_name} 또는 {wild_name} 아이콘)"
            )
...
def reward_text_ko(reward: Reward) -> str:
    ...
    match reward:
        ...
        case DeployFromGarrison(up_to=up_to):
            ...
            # Devious card face] — the player may deploy fewer; "Deploy (to
            # the Conflict) | 배치" [Main p. 10], [Main p. 20], "garrison |
            # 주둔지" [Bloodlines glossary row].
            return f"{{garrison}}에서 {{troop:{up_to}}} 이하를 {{conflict}}에 배치"
        case TrashPersonalCard(
            ...
            text = "{hand}의 카드 1장 {trash}" if hand_only else "카드 1장 {trash}"
            if bonus_spice:
                text += (
                    f"; 비용이 {bonus_minimum_cost} 이상이면: "
                    f"{{spice:{bonus_spice}}}"
                )
            return text
...
def option_text_ko(option: IntrigueOption, *, show_timing: bool = True) -> str:
    ...
    exactly as it does on the English one. ``show_timing`` is false for
    Steersman Y'rkoon's Navigation cards, the same as the English renderer:
    their faces print no Plot/Combat/Endgame banner (they are played
    automatically by Plot Course, not chosen by timing) [Navigation card
    faces].
    """

    body = "; ".join(section_text_ko(section) for section in option.sections)
    if option.trigger is not None:
        body = f"{trigger_text_ko(option.trigger)}: {body}"
    if not show_timing:
        return body
    return f"{_TIMING_LABELS_KO[option.timing]} — {body}"
...
def intrigue_card_text_ko(entry: IntrigueCardEntry) -> list[str]:
    ...
    ``describeAction()``). A Navigation card prints no timing banner --
    Plot Course plays it when its owner reaches 2 Influence [Steersman
    Y'rkoon card] -- so its lines carry no timing prefix, the same as
    ``intrigue_card_text``.
    """

    return [
        option_text_ko(option, show_timing=not entry.navigation)
