...
LEADER_FACE_TEXTS_KO: Mapping[str, LeaderFaceTextKo] = MappingProxyType(
    {
        ...
        "chani": LeaderFaceTextKo(
            ...
            # English now agrees ("Draw 2 cards"), and
            # rules/leader_abilities.py's _apply_chani_water_payment draws 2
            # cards too (both corrected on master). The water icon has no
            # printed number.
            signet_text=(
        ...
        "count_hasimir_fenring": LeaderFaceTextKo(
            ...
            # icon as the Deliver Supplies contract's Spy with Deep Cover;
            # display/leaders.py's English now agrees ("place a Spy with Deep
            # Cover on an Emperor observation post"), and
            # _leader_spy_placement_actions on EMPEROR_POST_IDS now passes
            # deep_cover=True too (both corrected on master). No dedicated
