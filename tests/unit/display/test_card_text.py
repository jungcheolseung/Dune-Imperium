...
def test_reliable_informant_ko_lists_its_spy_target_factions() -> None:
    ...
    # Re-read from the card face 2026-09-26: the third target icon is the
    # blue Fremen sietch badge, not the red Spacing Guild infinity symbol
    # (matches ``test_reliable_informant_lists_its_spy_target_factions``'s
    # English golden above).
    assert personal_card_text_ko(entry) == [
        "에이전트 칸: {spy} 배치 (황제, 베네 게세리트 또는 프레멘 {spy})",
        "공개 칸: {solari:1}",
    ]
...
def test_the_spice_must_flow_ko_reserve_acquisition_vp() -> None:
    ...
    # The Reveal box prints an orange spice hexagon "1" [Main p. 20] (icon
    # guide: spice, not a sword), not the previously transcribed sword
    # (matches ``test_the_spice_must_flow_reserve_acquisition_vp``'s
    # English golden above).
    assert personal_card_text_ko(entry) == [
        "공개 칸: {spice:1}",
        "획득 시: {victory_point:1}",
    ]
